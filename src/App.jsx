...
const { 
  ...
  Bell, Download, Upload, Search, MessageSquare, Clock, MoreHorizontal, ChevronRight, ChevronLeft,
  LogOut, Home, Menu, X, Scan, Plus, Minus, Trash2, Smartphone, FileText, CreditCard, ClockIcon,
  Wallet, QrCode, History, Star, Zap, User, Scissors, Database, Contact, FileBarChart, FileCheck, ClipboardCheck, CheckCircle,
  CheckCircle2, Camera, Dog, Cat, Rabbit, Megaphone, MessageCircle, PawPrint, Filter, Mail,
  ChevronDown, ChevronUp, Fish, Bird, Save, Code, HeartPulse, Activity, ShoppingCart, List, Grid,
  AlertCircle, AlertTriangle, Truck, FileInput, FileOutput, XCircle, ArrowLeft, ShoppingBasket, CheckSquare, Square,
  TrendingUp, Percent, Award, Check, Printer, RefreshCcw, Ban, FileSearch, Lock, Unlock, Coins, Banknote, Power,
  // ส่วนที่มีการเปลี่ยนชื่อ (Alias) ต้องใช้รูปแบบ Key: NewName
  Calendar: CalendarIcon, 
  Clock3, UserCheck, MapPin, Edit2, 
  Map: MapIcon, 
  Image: ImageIcon, 
  Link: LinkIcon, 
  Copy, ShieldCheck, Globe,
  Trophy, Hexagon, SlidersHorizontal, Briefcase, Share2
} = LucideIcons;
...
const CUSTOMERS_MOCK = [
  { 
    ...
    history: [
        { date: '10/12/2023', service: 'อาบน้ำตัดขน', groomer: 'ช่างแนน', price: 1200 },
        { date: '15/11/2023', service: 'อาบน้ำ', groomer: 'ช่างบอย', price: 600 }
    ]
  },
  { 
...
const MOCK_PURCHASE_ORDERS = [
    { id: 'PO-202406-003', date: '12/06/2024', supplier: 'Royal Canin Thailand', total: 13680, status: 'Pending', items: [
        { ...PetProductsDB[0], qty: 6 }
    ]},
    { id: 'PO-202406-002', date: '11/06/2024', supplier: 'Perfect Companion', total: 6480, status: 'Received', items: [
        { ...PetProductsDB[1], qty: 30 }
    ]},
    { id: 'PO-202406-001', date: '10/06/2024', supplier: 'Pet Care Supply', total: 2400, status: 'Cancelled', items: [
        { ...PetProductsDB[2], qty: 30 }
    ]},
    { id: 'PO-202405-099', date: '30/05/2024', supplier: 'Boehringer Ingelheim', total: 12000, status: 'Completed', items: [
...
// --- SERVICE & STAFF DATA ---
const SERVICES_DB = [
  { id: 'SVC001', name: 'อาบน้ำ - สุนัขเล็ก (S)', price: 350, duration: 60, type: 'dog' },
  { id: 'SVC002', name: 'อาบน้ำ - สุนัขกลาง (M)', price: 500, duration: 90, type: 'dog' },
  { id: 'SVC003', name: 'อาบน้ำ - สุนัขใหญ่ (L)', price: 800, duration: 120, type: 'dog' },
  { id: 'SVC004', name: 'อาบน้ำตัดขน - สุนัขเล็ก', price: 600, duration: 90, type: 'dog' },
  { id: 'SVC005', name: 'อาบน้ำตัดขน - สุนัขใหญ่', price: 1200, duration: 150, type: 'dog' },
  { id: 'SVC006', name: 'อาบน้ำ - แมว', price: 450, duration: 60, type: 'cat' },
  { id: 'SVC007', name: 'ตัดขน - แมว (ไม่วางยา)', price: 700, duration: 90, type: 'cat' },
  { id: 'SVC008', name: 'สปา + นวดผ่อนคลาย', price: 300, duration: 30, type: 'all' },
];

const STAFF_DB = [
  { id: 'ST001', name: 'ช่างส้ม', avatar: 'S' },
  { id: 'ST002', name: 'ช่างเล็ก', avatar: 'L' },
  { id: 'ST003', name: 'ช่างใหญ่', avatar: 'Y' },
];

const INITIAL_APPOINTMENTS = [
  { id: 1, staffId: 'ST001', memberName: 'คุณ วิภา', petName: 'มาร์นี่', breed: 'Persian', coatType: 'ยาว', weight: 4.2, service: 'อาบน้ำ - แมว', time: '10:00', duration: 60, status: 'Confirmed' },
  { id: 2, staffId: 'ST002', memberName: 'คุณ สมชาย', petName: 'หมูตุ๋น', breed: 'Golden', coatType: 'ยาวมาก', weight: 28.5, service: 'อาบน้ำตัดขน - สุนัขใหญ่', time: '13:00', duration: 150, status: 'Checked-in' },
  { id: 3, staffId: 'ST003', memberName: 'คุณ แป้ง', petName: 'ศรีตรัง', breed: 'Siamese', coatType: 'สั้น', weight: 3.5, service: 'ตัดขน - แมว', time: '11:00', duration: 90, status: 'Cancelled' },
  { id: 4, staffId: 'ST001', memberName: 'คุณ บาส', petName: 'โอเลี้ยง', breed: 'Poodle', coatType: 'หยิก', weight: 5.5, service: 'อาบน้ำ - สุนัขเล็ก', time: '13:00', duration: 60, status: 'Confirmed' },
];
...
const DRAWER_MENU_ITEMS = [
  ...
  { id: 'grooming', title: 'การจอง อาบน้ำ/ตัดขน', icon: CalendarDays },
...
// --- UTILITY DROPDOWN FILTER (Defined outside App to be usable by BillHistoryView) ---
    ...
    return (
        <div className="relative shrink-0" ref={dropdownRef}>
            <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-xl text-gray-700 font-bold hover:bg-gray-50 min-w-[140px] justify-between shadow-sm text-sm ${FONTS.header}`}
            >
                <span>{displayValue}</span>
                <ChevronDown size={14} className={`transition-transform ${isLocalOpen ? 'rotate-180' : ''}`}/>
            </button>
            
            {isLocalOpen && (
                <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-1 z-50 animate-in fade-in zoom-in duration-200 origin-top-right">
                    {options.map(option => (
                        <button
                            key={option}
                            onClick={() => {
                            ...
                            className={`w-full text-left px-4 py-2 hover:bg-gray-50 text-sm font-medium ${currentValue === option ? 'text-blue-600 bg-blue-50' : 'text-gray-700'}`}
                        >
                            {option}
                        </button>
                    ))}
...
// ==========================================
// ZONE E: CRM MODULE (MEMBER INFO ADDED)
// ==========================================
...
const POSSidebar = ({ onOpenDrawer, onNavigate, activeTab, onTabChange }) => {
  ...
  return (
    <div className="w-20 h-full relative z-40 shrink-0">
        <div className={`absolute top-0 left-0 h-full bg-white shadow-xl transition-all duration-300 flex flex-col border-r border-gray-100 ${isExpanded ? 'w-64' : 'w-20'}`} onMouseEnter={() => setIsExpanded(true)} onMouseLeave={() => setIsExpanded(false)}>
        <div className="flex flex-col h-full py-4">{menuItems.map((item, index) => (<div key={index} onClick={item.action} className={`relative flex items-center px-6 py-4 cursor-pointer transition-colors duration-200 ${activeTab === item.id ? 'bg-emerald-50 text-emerald-600 border-l-4 border-emerald-500' : ''} ${activeTab !== item.id && !item.highlight ? 'text-gray-400 hover:bg-gray-50 hover:text-emerald-500' : ''} ${item.highlight ? 'text-gray-800 hover:bg-gray-100' : ''} ${item.bottom ? 'mt-auto' : ''}`}><item.icon size={24} strokeWidth={activeTab === item.id ? 2.5 : 2} className={item.highlight ? 'text-gray-800' : ''} />{item.badge && !isExpanded && (<span className="absolute top-2 right-4 w-4 h-4 bg-red-500 rounded-full border-2 border-white"></span>)}<span className={`ml-4 ${FONTS.header} font-medium whitespace-nowrap overflow-hidden transition-all duration-300 ${isExpanded ? 'opacity-100 max-w-full' : 'opacity-0 max-w-0'} ${activeTab === item.id ? 'text-emerald-600 font-bold' : ''}`}>{item.label}</span>{item.badge && isExpanded && (<span className="ml-auto bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">{item.badge}</span>)}</div>))}</div>
        </div>
    </div>
  );
};
...
const CRMSidebar = ({ onOpenDrawer, onNavigate, activeTab, onTabChange }) => {
    const [isExpanded, setIsExpanded] = useState(false); // Changed default to false to match behavior
    const menuItems = [
      { icon: Menu, label: 'เมนูหลัก', action: onOpenDrawer, highlight: true },
...
// --- History Feature Components (New/Modified) ---

const BillHistoryView = () => {
    // Local States for filters and UI interaction
    const [searchTerm, setSearchTerm] = useState('');
    ...
    // --- Utility: Bill Filter Logic (Uses SSOT: SALES_HISTORY_DB) ---
    const filteredBills = useMemo(() => {
        let bills = SALES_HISTORY_DB;

        // 1. Search Filter (Bill ID, Member Name, Pet Name)
        if (searchTerm) {
            const term = searchTerm.toLowerCase();
            bills = bills.filter(bill => 
                bill.id.toLowerCase().includes(term) || 
                bill.member.toLowerCase().includes(term) ||
                (bill.pet && bill.pet.toLowerCase().includes(term)) || // Search Pet Name
        ...
        // 3. Time Range Filter (Simplified Mock Logic based on fixed dates: '11/06/2024' is today)
        if (timeRange !== 'ALL') {
            if (timeRange === 'Today') {
                bills = bills.filter(bill => bill.date === '11/06/2024');
            } else if (timeRange === 'Yesterday') {
                bills = bills.filter(bill => bill.date === '10/06/2024');
            } else if (timeRange === 'Last Month' || timeRange === '3 Months') {
                 // Mock: Return full data set for complex filters, for UI demonstration
            }
        }
    ...
    const summary = useMemo(() => {
        ...
        return {
            ...
            voidedBillCount: voidedBills.length,
        };
    }, [filteredBills]);
    ...
    const SummaryCard = ({ title, value, icon: Icon, color }) => (
        <div className="bg-white p-5 rounded-2xl shadow-md border border-gray-100 flex items-center justify-between min-w-[200px]">
            <div>
                <p className={`${FONTS.body} text-sm text-gray-500 font-medium`}>{title}</p>
                <p className={`${FONTS.header} text-2xl font-bold text-gray-800 tabular-nums`}>{value}</p>
            </div>
        ...
        <div className="flex-1 flex flex-col h-full bg-[#F3F4F6] overflow-hidden relative">
            <div className="h-24 bg-white border-b border-gray-200 px-8 flex items-center justify-between shrink-0 shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className={`${FONTS.header} text-2xl font-bold text-gray-800`}>ประวัติบิลขาย</h2>
                        <p className={`${FONTS.body} text-xs text-gray-400`}>ค้นหาและจัดการรายการขายย้อนหลัง</p>
                    </div>
                </div>
                ...
                <div className="flex gap-4">
                    <SummaryCard 
                        title="ยอดขายสุทธิ" 
                        value={`${SYSTEM_DEFAULTS.currencySymbol}${summary.totalSales.toLocaleString()}`} 
                        icon={TrendingUp} 
                        color="bg-emerald-500"
                    />
                    <SummaryCard 
                        title="จำนวนบิลทั้งหมด" 
                        value={summary.billCount.toLocaleString()} 
                        icon={FileText} 
                        color="bg-blue-500"
                    />
                    <SummaryCard 
                        title="บิลแคนเซิล (Void)" 
                        value={summary.voidedBillCount.toLocaleString()} 
                        icon={Ban} 
                        color="bg-rose-500"
                    />
                </div>
            ...
            <div className="p-8 pb-4 shrink-0">
                <div className="flex flex-wrap items-center gap-3 p-4 bg-white rounded-xl shadow-sm border border-gray-100">
                    ...
                    <div className="flex-1 relative min-w-[300px]">
                        ...
                        <input 
                            type="text" 
                            placeholder="ค้นหาเลขที่บิล, ชื่อลูกค้า, ชื่อสัตว์เลี้ยง..." 
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className={`${FONTS.header} w-full pl-12 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:border-blue-400 focus:ring-2 focus:ring-blue-100 outline-none transition text-gray-700 text-sm`} 
                        />
                    </div>
                    ...
                    <button className="px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl text-gray-600 font-bold hover:bg-gray-100 transition flex items-center gap-2 text-sm">
                        <Filter size={16} />
                        ขั้นสูง
                    </button>
                    
                    <button className="px-4 py-2 bg-blue-500 text-white rounded-xl font-bold hover:bg-blue-600 transition flex items-center gap-2 text-sm ml-auto">
                        <ArrowLeft size={16} />
                        เรียกคืนบิล
                    </button>
                </div>
                    ...
                    <div className="overflow-y-auto flex-1 custom-scrollbar">
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-gray-50 sticky top-0 z-10 text-xs text-gray-500 uppercase">
                                <tr>
                                    <th className="p-4 pl-6 font-bold">เลขที่บิล</th>
                                    <th className="p-4 font-bold">วันที่ / เวลา</th>
                                    <th className="p-4 font-bold">ลูกค้า / สัตว์เลี้ยง</th>
                                    <th className="p-4 font-bold text-center">รายการ</th>
                                    <th className="p-4 font-bold text-right">ยอดรวม</th>
                                    <th className="p-4 font-bold text-center">ชำระโดย</th>
                                    <th className="p-4 font-bold text-center">สถานะ</th>
                                    <th className="p-4 font-bold text-center">พนักงาน</th>
                                </tr>
                            </thead>
                                    ...
                                    filteredBills.map((bill) => (
                                        <tr key={bill.id} onClick={() => setSelectedBill(bill)} className="hover:bg-blue-50/50 transition cursor-pointer group">
                                            ...
                                            <td className="p-4 font-bold text-gray-800">
                                                <div>{bill.member}</div>
                                                <div className="text-xs text-gray-500 font-medium italic">{bill.pet || 'N/A'}</div>
                                            </td>
                                            ...
                                            <td className="p-4 text-center">
                                                <span className="px-3 py-1 rounded-full bg-gray-100 text-xs font-bold text-gray-600">{bill.method}</span>
                                            </td>
                                            <td className="p-4 text-center">
                                                <span className={`px-3 py-1 rounded-full text-xs font-bold ${bill.status === 'Completed' ? 'bg-emerald-100 text-emerald-600' : bill.status === 'Voided' ? 'bg-rose-100 text-rose-600' : 'bg-orange-100 text-orange-600'}`}>
                                                    {bill.status}
                                                </span>
                                            </td>
                                    ...
                                    <tr>
                                        <td colSpan="8" className="p-12 text-center text-gray-400">
                                            <div className="flex flex-col items-center justify-center">
                                                ...
                                                <p className={`${FONTS.header} text-lg font-medium`}>ไม่พบข้อมูลบิลที่ตรงกับเงื่อนไข</p>
                                                <p className={`${FONTS.body} text-sm`}>ลองปรับเปลี่ยนเงื่อนไขการค้นหาหรือช่วงเวลา</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
            ...
            {/* Bill Detail Popup (Copied from previous version) */}
...
const CashDrawerView = ({ settings }) => {
    ...
    const handleOpenDrawer = () => { if (pin === '1234') { console.log('ลิ้นชักเปิดแล้ว'); setShowPinModal(false); setPin(''); } else { console.log('รหัสผ่านผิด'); } }; // Changed alert to console.log
...
const PaymentModal = ({ isOpen, onClose, cartItems, total, member, onConfirm, settings }) => {
  ...
  // Calculate Earned Points (50 THB = 1 Point)
  const earnedPoints = Math.floor(total / 50);

  // Filter Service items for redemption (Assuming 1 Point = 1 THB discount for simplicity)
  // Logic: Can only redeem up to the value of "Service" items
  const serviceItemsTotal = cartItems
    .filter(item => item.type === 'service')
    .reduce((sum, item) => sum + ((item.price * item.qty) - (item.discount || 0)), 0);
    ...
    <div className="fixed inset-0 z-[99] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in duration-200 flex flex-col">
        <div className="bg-gray-900 p-6 text-white flex justify-between items-center">
          <div>
            <h3 className={`${FONTS.header} text-2xl font-bold`}>ชำระเงิน</h3>
            <p className={`${FONTS.body} text-gray-400 text-sm`}>กรุณาตรวจสอบยอดชำระ</p>
          </div>
        ...
        <div className="p-6 space-y-6">
          <div className="flex flex-col items-center justify-center py-2">
            <span className="text-gray-500 text-sm font-medium mb-1">ยอดชำระสุทธิ</span>
            <span className={`${FONTS.header} text-5xl font-bold text-emerald-600`}>{settings.currencySymbol}{finalTotal.toLocaleString()}</span>
            {pointsRedeem && <span className="text-xs text-emerald-500 font-bold mt-2 bg-emerald-50 px-2 py-1 rounded-lg">ลดราคาจากแต้ม: -{discountAmount.toLocaleString()}</span>}
          </div>
          ...
          <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-3">
             <div className="flex justify-between items-center">
                <div className="flex items-center gap-2">
                   ...
                   <span className="text-sm font-bold text-gray-700">คะแนนที่จะได้รับ</span>
                </div>
                <span className="text-lg font-bold text-amber-500">+{earnedPoints.toLocaleString()}</span>
             </div>
             <p className="text-xs text-gray-400 text-right">(คำนวณจากยอด 50 บาท = 1 คะแนน)</p>
          </div>
          ...
          <div className={`rounded-xl p-4 border transition-all ${pointsRedeem ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-200'}`}>
            <div className="flex justify-between items-center mb-2">
               <div className="flex items-center gap-2">
                  <Coins className={pointsRedeem ? "text-amber-600" : "text-gray-400"} size={20} />
                  <div>
                    <h4 className={`text-sm font-bold ${pointsRedeem ? 'text-amber-800' : 'text-gray-700'}`}>ใช้คะแนนแลกส่วนลด</h4>
                    <p className="text-xs text-gray-500">มีคะแนนสะสม: {member.points.toLocaleString()}</p>
                  </div>
               </div>
               <div 
                 onClick={() => {
                   if (serviceItemsTotal <= 0 && !pointsRedeem) {
                     alert("ไม่มีรายการ 'บริการ' ในบิลนี้ ไม่สามารถใช้คะแนนได้");
                     return;
                   }
                   if (member.points <= 0 && !pointsRedeem) {
                     alert("คะแนนสะสมไม่เพียงพอ");
                     return;
                   }
                   setPointsRedeem(!pointsRedeem);
                 }}
                 className={`w-12 h-6 rounded-full p-1 cursor-pointer transition-colors ${pointsRedeem ? 'bg-amber-500' : 'bg-gray-200'}`}
               >
                 <div className={`w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${pointsRedeem ? 'translate-x-6' : 'translate-x-0'}`}></div>
               </div>
            </div>
            
            {pointsRedeem && (
              <div className="text-xs text-amber-700 border-t border-amber-200/50 pt-2 mt-2">
                 <p>สามารถใช้ลดได้สูงสุด: <span className="font-bold">{maxRedeemablePoints.toLocaleString()} บาท</span></p>
                 <p className="opacity-75">(เฉพาะรายการค่าบริการ)</p>
              </div>
            )}
            {!pointsRedeem && serviceItemsTotal > 0 && member.points > 0 && (
                 <p className="text-xs text-gray-400 mt-1 pl-7">ใช้คะแนนลดได้สูงสุด {Math.min(member.points, serviceItemsTotal).toLocaleString()} บาท (เฉพาะบริการ)</p>
            )}
          </div>
        ...
        <div className="p-6 pt-0">
          <button 
            onClick={() => onConfirm(finalTotal, earnedPoints)} 
            className="w-full py-4 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-lg shadow-lg shadow-emerald-200 transition active:scale-95 flex items-center justify-center gap-2"
          >
...
const POSModule = ({ cartItems, setCartItems, currentMember, recommendedItems, handleNavigate, setIsDrawerOpen, setIsMemberSearchOpen, setIsNewMemberOpen, addToCart, calculateTotal, settings }) => {
  ...
  // --- Helper to get real pets from currentMember ---
  ...
  const handleHoldBill = () => {
    ...
    setCartItems([]);
    // In a real app, you might want to reset member to default too, but keeping for demo flow
  };

  const handleRecallBill = (bill) => {
    setCartItems(bill.items);
    // Restore member context if needed, for now just items
    ...
    <div className="flex-1 flex flex-col min-w-0">
      {activePOSPage === 'terminal' ? (
        <>
                ...
                <div className="flex items-center gap-8">
                  ...
                  <div className={`border rounded-xl px-4 py-2 flex items-center gap-3 transition-colors ${currentMember.levelColor || 'bg-gray-100 text-gray-500'} border-transparent`}>
                      <div className="text-right">
                          <p className={`${FONTS.header} text-xs font-bold opacity-80`}>ระดับสมาชิก</p>
                          <p className={`${FONTS.header} text-sm font-bold`}>★ {currentMember.level}</p>
                      </div>
                      <div className="h-8 w-px bg-current opacity-20"></div>
                      <div className="text-right">
                          <p className={`${FONTS.header} text-xs font-bold opacity-80`}>คะแนนสะสม</p>
                          <p className={`${FONTS.header} text-lg font-bold`}>{currentMember.points.toLocaleString()}</p>
                      </div>
                  </div>
                </div>
            </header>
            <div className="flex-1 p-3 flex gap-3 overflow-hidden">
                <div className="flex-1 bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
                    ...
                    <div className="grid grid-cols-12 gap-3 h-16">
                      <button 
                        ...
                        className="col-span-2 bg-white border border-orange-500 text-orange-600 rounded-xl font-bold hover:bg-orange-50 transition flex items-center justify-center gap-1"
                      >
                        <Power size={18} /> พักบิล
                      </button>
                      <button 
                        onClick={() => setShowRecallModal(true)}
                        className="col-span-2 bg-white border border-blue-500 text-blue-600 rounded-xl font-bold hover:bg-blue-50 transition flex items-center justify-center gap-1 relative"
                      >
                        <RefreshCcw size={18} /> เรียกบิล
                        {heldBills.length > 0 && <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs w-5 h-5 flex items-center justify-center rounded-full">{heldBills.length}</span>}
                      </button>
                      <div className="col-span-4 grid grid-cols-4 gap-2">
                        {[{ id: 'Cash', icon: Wallet, label: 'เงินสด' }, { id: 'Transfer', icon: CreditCard, label: 'โอนเงิน' }, { id: 'Scan', icon: QrCode, label: 'สแกน' }, { id: 'Wallet', icon: Smartphone, label: 'Wallet' }].map((m) => (
                          <button 
                            key={m.id} 
                            onClick={() => setSelectedPaymentMethod(m.id)}
                            className={`border rounded-xl flex flex-col items-center justify-center transition ${selectedPaymentMethod === m.id ? 'bg-emerald-50 border-emerald-500 text-emerald-600' : 'bg-white border-gray-200 text-gray-500 hover:border-emerald-500 hover:text-emerald-500'}`}
                          >
                            <m.icon size={18} />
                      ...
                      <button 
                        ...
                        className={`col-span-4 rounded-2xl text-xl font-bold shadow-lg transition-all active:scale-95 ${cartItems.length > 0 ? 'bg-emerald-500 hover:bg-emerald-600 text-white shadow-emerald-200' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
                      >
                        ชำระเงิน
                      </button>
                    </div>
                ...
                <div className="w-72 flex flex-col gap-2 shrink-0">
                    <div className="flex flex-wrap items-start justify-start gap-2 py-1 min-h-[50px] content-start">
                      {currentMemberPets.length > 0 ? (
                        currentMemberPets.map((pet, i) => (
                          <div key={i} className="flex flex-col items-center gap-1 cursor-pointer group w-[3.5rem]">
                            <div className="relative w-10 h-10 rounded-full bg-orange-100 border-2 border-white shadow-sm flex items-center justify-center text-lg group-hover:scale-110 transition">
                              {getPetEmoji(pet.type)}
                              {pet.chronicDiseases && pet.chronicDiseases.length > 0 && (
                                <div className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full text-white text-[8px] flex items-center justify-center border border-white">!</div>
                              )}
                            </div>
                            <span className={`${FONTS.header} text-[10px] font-bold text-gray-600 truncate w-full text-center`}>{pet.name}</span>
                          </div>
                        ))
                      ) : (
                         <div className="flex flex-col items-center justify-center w-full py-1 text-gray-400 opacity-50">
                           <PawPrint size={20} className="mb-0.5"/>
                           <span className="text-[10px]">ไม่พบสัตว์เลี้ยง</span>
                         </div>
                      )}
                      
                      <div className="flex flex-col items-center gap-1 cursor-pointer group w-[3.5rem]">
                        <div className="w-10 h-10 rounded-full border-2 border-dashed border-gray-300 flex items-center justify-center text-gray-400 group-hover:border-emerald-500 group-hover:text-emerald-500 transition bg-white">
                          <Plus size={18} />
                        </div>
                        <span className={`${FONTS.header} text-[10px] font-bold text-gray-400`}>เพิ่ม</span>
                      </div>
                    </div>
            ...
            {showRecallModal && (
              <div className="fixed inset-0 z-[99] flex items-center justify-center p-4">
                <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowRecallModal(false)}></div>
                <div className="relative bg-white w-full max-w-lg rounded-3xl shadow-xl p-6">
                   <div className="flex justify-between items-center mb-4">
                      <h3 className={`${FONTS.header} text-xl font-bold`}>รายการบิลที่พักไว้</h3>
                      <button onClick={() => setShowRecallModal(false)}><X size={24} className="text-gray-400" /></button>
                   </div>
                   <div className="space-y-2 max-h-[60vh] overflow-y-auto">
                      {heldBills.length === 0 ? (
                        <p className="text-center text-gray-400 py-8">ไม่มีบิลที่พักไว้</p>
                      ) : (
                        heldBills.map(bill => (
                          <div key={bill.id} className="flex justify-between items-center p-4 bg-gray-50 rounded-xl border border-gray-200">
                             <div>
                                <p className="font-bold text-gray-800">บิลเวลา {bill.timestamp.toLocaleTimeString()}</p>
                                <p className="text-sm text-gray-500">{bill.items.length} รายการ | ลูกค้า: {bill.member.name}</p>
                             </div>
                             <button 
...
// ==========================================
// GROOMING MODULE SUB-COMPONENTS
// ==========================================

const GroomingSidebar = ({ onNavigate, activePage, onChangePage }) => {
  const menuItems = [
  ...
  return (
    <div className="w-64 bg-[#1E1E24] h-full flex flex-col flex-shrink-0 z-20 shadow-xl transition-all duration-300">
      {/* Header */}
      <div className="h-16 flex items-center px-6 border-b border-gray-800 bg-[#1E1E24]">
        <div className="flex items-center gap-3">
           ...
           <div>
             <span className={`${FONTS.header} font-bold text-lg text-white block leading-none`}>Pet Salon</span>
           </div>
        </div>
      </div>
          ...
          return (
            <button 
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                isActive 
                  ? 'bg-[#FCD34D] text-gray-900 shadow-md font-bold' 
                  : 'text-gray-400 hover:bg-gray-800 hover:text-gray-100'
              }`}
            >
              <item.icon size={18} className={isActive ? 'text-gray-900' : 'text-gray-500 group-hover:text-gray-300'} />
              <span className={`${FONTS.header}`}>{item.label}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className="p-4 border-t border-gray-800 bg-[#1E1E24]">
         {menuItems.filter(i => i.bottom).map(item => (
            <button key={item.id} onClick={() => onChangePage(item.id)} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium text-gray-400 hover:bg-gray-800 hover:text-white transition-all">
                <item.icon size={18} /> <span className={`${FONTS.header}`}>{item.label}</span>
            </button>
         ))}
      </div>
    </div>
...
const BookingCalendar = ({ appointments, setAppointments, onOpenModal }) => {
  ...
  const getPosition = (time) => {
    if (!time) return 0; // Guard clause to prevent split of undefined
                 ...
                 <div className="relative h-[1200px] bg-white"> 
                    ...
                    {appointments.filter(app => app.groomerId === groomer.id).map(app => {
                        ...
                        return (
                            <div 
                                key={app.id}
                                onClick={(e) => { e.stopPropagation(); onOpenModal(app); }}
                                ...
                                style={{ 
                                    top: `${getPosition(app.startTime)}px`, 
                                    height: `${getHeight(app.startTime, app.endTime)}px` 
                                }}
                            >
                                <div className="flex flex-col h-full">
                                    <div className="flex justify-between items-start">
                                        <span className={`${FONTS.header} font-bold text-sm text-gray-800 line-clamp-1`}>{customer?.pet?.name || 'ลูกค้าทั่วไป'}</span>
                                    </div>
                                    <div className="flex items-center gap-1 mt-0.5">
                                        <Scissors size={10} className="text-gray-500" />
                                        <span className="text-[10px] text-gray-600 line-clamp-1">{service?.name}</span>
                                    </div>
                                    <div className="mt-auto flex items-center justify-between">
                                        <span className="text-[10px] bg-white/60 px-1.5 py-0.5 rounded text-gray-600 font-medium">{app.startTime} - {app.endTime}</span>
                                    </div>
                                </div>
                            </div>
                        );
...
// --- 3. Groomer Profile & Settings ---

const GroomerDetail = ({ groomer, onBack }) => {
    const [activeTab, setActiveTab] = useState('profile'); // profile, schedule, settings
            ...
            <div className="bg-white border-b border-gray-200 p-6 flex justify-between items-start">
                <div className="flex items-center gap-6">
                    ...
                    <div className="w-24 h-24 rounded-2xl overflow-hidden border-4 border-white shadow-lg">
                        <img src={groomer.image} className="w-full h-full object-cover" />
                    </div>
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <h2 className={`${FONTS.header} text-2xl font-bold text-gray-900`}>{groomer.name}</h2>
                            {groomer.reviewsCount > 50 && (
                                <span className="bg-indigo-100 text-indigo-700 text-xs px-2 py-0.5 rounded-full border border-indigo-200 font-bold flex items-center gap-1">
                                    <Award size={12} /> Top Groomer
                                </span>
                            )}
                        </div>
                        <p className="text-gray-500 font-medium mb-3">"{groomer.nickname}"</p>
                        <div className="flex gap-2">
                             {groomer.awards.map((award, i) => (
                                 <span key={i} className="bg-yellow-50 text-yellow-700 text-xs px-2 py-1 rounded border border-yellow-200 font-bold flex items-center gap-1">
                                     <Trophy size={12} className="fill-yellow-500 text-yellow-500" /> {award}
                                 </span>
                             ))}
                        </div>
                    </div>
                ...
                <div className="flex gap-2">
                    <button onClick={()=>setActiveTab('profile')} className={`px-4 py-2 rounded-lg font-bold text-sm transition ${activeTab==='profile' ? 'bg-gray-900 text-white' : 'bg-white border hover:bg-gray-50'}`}>Profile</button>
                    <button onClick={()=>setActiveTab('settings')} className={`px-4 py-2 rounded-lg font-bold text-sm transition ${activeTab==='settings' ? 'bg-gray-900 text-white' : 'bg-white border hover:bg-gray-50'}`}>Settings</button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8">
                {activeTab === 'profile' ? (
                    <div className="grid grid-cols-12 gap-8">
                        ...
                        <div className="col-span-4 space-y-6">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Hexagon size={18}/> Skill Radar</h3>
                                {/* Replaced Missing RadarChart */}
                                <div className="h-40 bg-gray-50 rounded-xl flex items-center justify-center text-gray-400 text-xs">
                                    Radar Chart Placeholder
                                </div>
                            </div>
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Tag size={18}/> Specialization</h3>
                                <div className="flex flex-wrap gap-2">
                                    {groomer.skills.map(skill => (
                                        <span key={skill} className="px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-sm hover:bg-gray-200 cursor-default">{skill}</span>
                                    ))}
                                    <button className="px-3 py-1 border border-dashed border-gray-300 text-gray-400 rounded-full text-sm hover:border-gray-400 hover:text-gray-500">+ Add</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="col-span-8 space-y-6">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                                <h3 className="font-bold text-gray-800 mb-6 flex items-center gap-2"><History size={18}/> Work History</h3>
                                <div className="space-y-4">
                                    {[1,2,3].map(i => (
                                        <div key={i} className="flex justify-between items-center p-4 border border-gray-100 rounded-xl hover:bg-gray-50 transition">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 bg-indigo-50 rounded-lg flex items-center justify-center text-2xl">🐩</div>
                                                <div>
                                                    <p className="font-bold text-gray-800">น้องมอมแมม (Poodle)</p>
                                                    <p className="text-xs text-gray-500">ตัดขนทรงมาตรฐาน • 12/12/2023</p>
                                                </div>
                                            </div>
                                            <div className="text-right">
                                                <div className="flex text-yellow-400 mb-1"><Star size={12} fill="currentColor"/><Star size={12} fill="currentColor"/><Star size={12} fill="currentColor"/><Star size={12} fill="currentColor"/><Star size={12} fill="currentColor"/></div>
                                                <p className="text-xs text-gray-400">"ช่างมือเบามากค่ะ"</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                    ...
                    <div className="max-w-3xl mx-auto space-y-6">
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                            <h3 className="font-bold text-gray-800 mb-6 flex items-center gap-2"><SlidersHorizontal size={18}/> Work Settings</h3>
                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-2">Commission Rate (%)</label>
                                    <input type="number" defaultValue={groomer.commissionRate} className="w-full px-4 py-2 border border-gray-200 rounded-xl" />
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-2">Daily Wage (Optional)</label>
                                    <input type="number" placeholder="0.00" className="w-full px-4 py-2 border border-gray-200 rounded-xl" />
                                </div>
                                <div className="col-span-2">
                                    <label className="block text-sm font-bold text-gray-700 mb-2">Working Schedule</label>
                                    <div className="flex gap-2">
                                        {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map(d => (
                                            <button key={d} className={`flex-1 py-2 rounded-lg border ${groomer.schedule.includes(d) || ['Mon','Tue','Wed','Thu','Fri'].includes(d) ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-400 border-gray-200'}`}>{d}</button>
                                        ))}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                             <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Briefcase size={18}/> Portability</h3>
                             <p className="text-sm text-gray-500 mb-4">Groomer สามารถย้ายร้านได้โดยการ Transfer Profile ID ข้อมูลประวัติการทำงานและรีวิวจะถูกย้ายตามไปด้วย</p>
                             <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 flex justify-between items-center">
                                 <code className="text-lg font-mono font-bold text-gray-700">GROOM-ID-{groomer.id}-X89</code>
                                 <button className="text-indigo-600 font-bold text-sm flex items-center gap-2 hover:bg-indigo-50 px-3 py-1.5 rounded-lg transition"><Share2 size={16}/> Transfer Profile</button>
                             </div>
                        </div>
                    </div>
                )}
...
const GroomingModule = ({ onNavigate, setIsDrawerOpen }) => {
    ...
    // Logic to Merge new services (Update or Create)
    const handleSaveServices = (newServices) => {
        setServices(prev => {
            const serviceMap = new Map(prev.map(s => [s.id, s]));
            newServices.forEach(s => serviceMap.set(s.id, s)); // Overwrite if exists
             ...
             return (
                 <div className="p-8 h-full bg-gray-50 flex flex-col">
                     <div className="flex justify-between items-center mb-6">
                         ...
                         <button onClick={() => setIsQuickSetupOpen(true)} className="bg-white text-indigo-600 border border-indigo-200 px-4 py-2 rounded-xl font-bold hover:bg-indigo-50 flex items-center gap-2 shadow-sm">
                         ...
                         <div className="overflow-y-auto flex-1 custom-scrollbar">
                             <table className="w-full">
                                 <thead className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                                     <tr>
                                         <th className="p-4 text-left font-bold text-gray-600">Service Name</th>
                                         <th className="p-4 text-left font-bold text-gray-600">Type</th>
                                         <th className="p-4 text-left font-bold text-gray-600">Duration</th>
                                 ...
                                 <tbody className="divide-y divide-gray-100">
                                     {services.map(s => (
                                         <tr key={s.id} className="hover:bg-gray-50">
                                             ...
                                             <td className="p-4"><span className={`px-2 py-1 rounded text-xs font-bold uppercase ${s.type==='dog' ? 'bg-indigo-50 text-indigo-600' : 'bg-pink-50 text-pink-600'}`}>{s.type}</span></td>
                                             <td className="p-4 text-gray-500">{s.duration} min</td>
                                             <td className="p-4 text-right font-bold text-emerald-600">{s.price} ฿</td>
                                             <td className="p-4 text-right text-gray-400"><MoreHorizontal size={18} /></td>
                                         </tr>
                                     ))}
                                 </tbody>
                             </table>
             ...
             return (
                 <div className="p-8 h-full bg-gray-50">
                      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 h-full flex flex-col">
                          <h2 className={`${FONTS.header} text-2xl font-bold text-gray-800 mb-6`}>Client History</h2>
                          <div className="flex-1 overflow-y-auto">
                              {CUSTOMERS_MOCK.map(c => (
                                  <div key={c.id} className="border-b border-gray-100 py-4 last:border-0">
                                      <div className="flex justify-between items-start mb-2">
                                          <div className="flex items-center gap-4">
                                               <div className="text-3xl">{c.pet.img}</div>
                                               <div>
                                                   <h4 className="font-bold text-lg text-gray-900">{c.pet.name} <span className="text-sm font-normal text-gray-500">({c.pet.breed})</span></h4>
                                                   <p className="text-sm text-gray-500">Owner: {c.name} • {c.phone}</p>
                                               </div>
                                          </div>
                                          <button className="text-indigo-600 text-sm font-bold">View Details</button>
                                      </div>
                                      {c.history && c.history.length > 0 && (
                                          <div className="ml-14 bg-gray-50 p-3 rounded-xl text-sm space-y-2 mt-2">
                                              {c.history.map((h, i) => (
                                                  <div key={i} className="flex justify-between text-gray-600">
                                                      <span>{h.date} - {h.service} (by {h.groomer})</span>
    ...
    return (
        <div className="h-screen w-full bg-[#F3F4F6] flex overflow-hidden">
            {/* Sidebar */}
            <GroomingSidebar onNavigate={onNavigate} activePage={activePage} onChangePage={(id) => { setSelectedGroomer(null); setActivePage(id); }} />
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 flex-shrink-0 z-10 shadow-sm">
                    <div className="flex items-center gap-4">
                        ...
                        <h1 className={`${FONTS.header} text-xl font-bold text-gray-800 capitalize`}>
                            {activePage === 'booking' ? 'ตารางนัดหมาย' : activePage}
                        </h1>
                    </div>
                    <div className="flex items-center gap-4">
            ...
            <AppointmentModal 
                ...
                onSave={(data) => { handleSaveAppointment(data); setIsBookingModalOpen(false); }}
                onDelete={() => {}}
                services={services} // Pass latest services to modal
            />
...
// ==========================================
// REUSED APPOINTMENT MODAL (UPDATED)
// ==========================================

const AppointmentModal = ({ isOpen, onClose, appointment, onSave, onDelete, services = [] }) => {
    if (!isOpen) return null;
    
    // Use passed services or fallback to mock if undefined (though it should be passed)
    ...
    const [formData, setFormData] = useState({
        ...
        serviceId: '',
        startTime: '10:00',
        endTime: '11:00', // Added default endTime
        weight: '',
        note: ''
    });
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
            ...
            <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl relative z-10 flex flex-col max-h-[90vh]">
                 <div className="flex justify-between items-center p-6 border-b border-gray-100">
                    <h2 className={`${FONTS.header} text-xl font-bold text-gray-800`}>
                        {appointment?.id ? 'แก้ไขนัดหมาย' : 'ลงนัดหมายใหม่'}
                    </h2>
                ...
                <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-6">
                    ...
                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                        <div className="flex justify-between mb-3">
                             <label className="block text-sm font-bold text-gray-700">ข้อมูลลูกค้า</label>
                             <button onClick={() => setIsNewCustomer(!isNewCustomer)} className="text-xs text-indigo-600 font-bold underline">
                                 {isNewCustomer ? 'ค้นหาลูกค้าเดิม' : '+ สร้างลูกค้าใหม่'}
                             </button>
                        </div>
                        
                        {isNewCustomer ? (
                             <div className="grid grid-cols-2 gap-3">
                                 <input placeholder="ชื่อลูกค้า" className="px-3 py-2 border rounded-lg" />
                                 <input placeholder="เบอร์โทร" className="px-3 py-2 border rounded-lg" />
                                 <input placeholder="ชื่อสัตว์เลี้ยง" className="px-3 py-2 border rounded-lg" />
                                 <input placeholder="สายพันธุ์" className="px-3 py-2 border rounded-lg" />
                             </div>
                        ) : (
                             <div className="relative">
                                 <Search className="absolute left-3 top-3 text-gray-400" size={16} />
                                 <input type="text" placeholder="ค้นหาลูกค้า..." className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg outline-none focus:ring-2 focus:ring-indigo-100" />
                             </div>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                             <label className="block text-sm font-bold text-gray-700 mb-2">ช่างตัดขน</label>
                             <select 
                        ...
                        <div>
                             ...
                             <input type="time" value={formData.startTime} onChange={(e) => setFormData({...formData, startTime: e.target.value})} className="w-full px-4 py-2.5 border border-gray-200 rounded-xl" />
                        </div>
                    </div>

                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-2">เลือกบริการ</label>
                        <div className="grid grid-cols-1 gap-2 max-h-40 overflow-y-auto border border-gray-100 rounded-xl p-2 bg-white">
                            {availableServices.map(s => (
                                <label key={s.id} className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-all ${formData.serviceId === s.id ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' : 'bg-white border-gray-200 hover:border-indigo-300 text-gray-700'}`}>
                                    <div className="flex items-center gap-3">
                                        <input type="radio" name="service" className="hidden" checked={formData.serviceId === s.id} onChange={() => setFormData({...formData, serviceId: s.id})} />
                                        <div><p className="font-bold text-sm">{s.name}</p><span className="text-xs opacity-80">{s.duration} mins</span></div>
                                    </div>
                ...
                <div className="p-6 border-t border-gray-100 bg-gray-50 rounded-b-2xl flex justify-end gap-3">
                     <button onClick={onClose} className="px-6 py-2.5 rounded-xl text-gray-500 font-bold hover:bg-gray-200 transition">ยกเลิก</button>
                     <button onClick={() => onSave(formData)} className="px-8 py-2.5 rounded-xl bg-indigo-600 text-white font-bold shadow-lg shadow-indigo-200 hover:bg-indigo-700 transition">บันทึกนัดหมาย</button>
                </div>
            </div>
        </div>
    );
...
const GoodsReceiptListView = ({ onSelectGR }) => {
    const [filterStatus, setFilterStatus] = useState('All');
    const [searchTerm, setSearchTerm] = useState('');

    const filteredGRs = MOCK_GOODS_RECEIPTS.filter(gr => {
                ...
                <div className="flex justify-between items-center gap-4">
                        ...
                        {['All', 'Pending Inspection', 'Completed', 'Discrepancy'].map(status => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-lg text-sm transition-all ${
                                    filterStatus === status 
                                    ? 'bg-gray-900 text-white font-bold shadow-md' // <-- Active: พื้นดำ ตัวขาว ตัวหนา
                    ...
                    <div className="relative w-64">
                         ...
                         <input 
                            ...
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pl-10 pr-4 py-2 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-100 focus:border-emerald-500 outline-none"
                         />
                    </div>
                </div>
             </div>
             ...
             <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                <div className="grid grid-cols-1 gap-4">
                    {filteredGRs.map(gr => (
                        <div 
                            key={gr.id}
                            onClick={() => onSelectGR(gr)}
                            className="bg-white p-5 rounded-2xl border border-gray-200 hover:border-emerald-400 hover:shadow-md transition-all cursor-pointer group"
                        >
                            <div className="flex justify-between items-center mb-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className={`${FONTS.header} text-lg font-bold text-gray-800`}>{gr.id}</h3>
                                        <p className={`${FONTS.body} text-sm text-gray-400 flex items-center gap-1`}>
                                            Ref PO: <span className="text-gray-600 font-mono">{gr.poId}</span> • <ClockIcon size={12}/> {gr.date}
                                        </p>
                                    </div>
                                </div>
                                <div className={`px-3 py-1 rounded-full text-xs font-bold border ${getStatusColor(gr.status)}`}>
                                    {gr.status}
                                </div>
                            </div>
                            <div className="flex justify-between items-end border-t border-gray-100 pt-4">
                                <div>
                                    <p className="text-xs text-gray-400 mb-1">ผู้จัดจำหน่าย</p>
                                    <p className="text-sm font-medium text-gray-700">{gr.supplier}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs text-gray-400 mb-1">จำนวนรายการ</p>
                                    <p className={`${FONTS.header} text-xl font-bold text-emerald-600`}>{gr.totalItems} หน่วย</p>
                                </div>
                            </div>
                        </div>
                    ))}
...
// --- Goods Receipt Detail View (Inspection & Final Doc) ---
const GoodsReceiptDetailView = ({ grData, onClose }) => {
    const [mode, setMode] = useState(grData.status === 'Pending Inspection' ? 'inspect' : 'final'); // 'inspect', 'final'
    const [inspectionItems, setInspectionItems] = useState(grData.items.map(item => ({
        ...item,
        actualQty: item.qty // Pre-fill with expected
    })));
    ...
    // Sending State (for final view)
    const [sendingStatus, setSendingStatus] = useState(null);

    // Helper to calculate sub-unit quantity
    const getSubUnitInfo = (item, qty) => {
        ...
        return {
            totalSubUnits,
            subUnit,
            display: item.packDetail ? `${qty} ${item.unit} (${totalSubUnits} ${subUnit})` : `${qty} ${item.unit}`
        };
    };
    ...
    const handleConfirmOtp = () => {
        if (otp === '1234') { // Mock OTP
            ...
            setOtpError('รหัส OTP ไม่ถูกต้อง กรุณาขอจากผู้ขายอีกครั้ง');
        }
    };
        ...
        return (
            <div className="flex flex-col h-full bg-[#F9FAFB] relative">
                 ...
                 <div className="px-8 py-5 border-b border-gray-200 bg-white flex justify-between items-center sticky top-0 z-20">
                     <div className="flex items-center gap-4">
                         <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full transition text-gray-500"><ArrowLeft size={24}/></button>
                         <div>
                             <h2 className={`${FONTS.header} text-xl font-bold text-gray-800 flex items-center gap-2`}>
                                <ClipboardCheck className="text-emerald-600" /> ตรวจรับสินค้า (Inspection)
                             </h2>
                             <p className={`${FONTS.body} text-sm text-gray-400`}>{grData.id} • {grData.supplier}</p>
                         </div>
                     </div>
                     <button onClick={handleSaveInspection} className="px-6 py-2.5 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 shadow-lg shadow-emerald-200 transition">
                         บันทึกการตรวจรับ
                     </button>
                 </div>
                     ...
                     <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                         <table className="w-full">
                             <thead className="bg-gray-50 border-b border-gray-200">
                                 <tr>
                                     <th className={`${FONTS.header} py-4 pl-6 text-left text-xs font-bold text-gray-500`}>สินค้า</th>
                                     <th className={`${FONTS.header} py-4 text-center text-xs font-bold text-gray-500`}>จำนวนสั่งซื้อ (Expected)</th>
                                     <th className={`${FONTS.header} py-4 text-center text-xs font-bold text-gray-500`}>จำนวนรับจริง (Actual)</th>
                                     <th className={`${FONTS.header} py-4 text-center text-xs font-bold text-gray-500`}>สถานะ</th>
                                 </tr>
                             </thead>
                             <tbody className="divide-y divide-gray-100">
                                 {inspectionItems.map((item) => {
                                     ...
                                     return (
                                         <tr key={item.id} className={!isMatch ? 'bg-red-50/30' : ''}>
                                             <td className="py-4 pl-6">
                                                 <p className={`${FONTS.header} text-gray-800 font-bold text-sm`}>{item.name}</p>
                                                 <p className={`${FONTS.body} text-gray-500 text-xs`}>{item.barcode}</p>
                                             </td>
                                             <td className="py-4 text-center">
                                                 <span className="text-gray-800 font-medium">{expectedInfo.display}</span>
                                             </td>
                                             <td className="py-4 text-center">
                                                 <div className="flex justify-center items-center gap-2">
                                                     <button onClick={() => handleActualQtyChange(item.id, Math.max(0, item.actualQty - 1))} className="p-1 rounded bg-gray-100 hover:bg-gray-200 text-gray-600"><Minus size={14}/></button>
                                                     <input 
                                                        type="number" 
                                                        value={item.actualQty}
                                                        onChange={(e) => handleActualQtyChange(item.id, e.target.value)}
                                                        className={`w-16 text-center border rounded-lg py-1 font-bold ${!isMatch ? 'border-red-300 text-red-600 bg-white' : 'border-gray-200 text-gray-800'}`}
                                                     />
                                                     <button onClick={() => handleActualQtyChange(item.id, item.actualQty + 1)} className="p-1 rounded bg-gray-100 hover:bg-gray-200 text-gray-600"><Plus size={14}/></button>
                                                 </div>
                                                 <p className="text-xs text-gray-400 mt-1">({actualInfo.totalSubUnits} {actualInfo.subUnit})</p>
                                             </td>
                                             <td className="py-4 text-center">
                                                 {isMatch ? (
                                                     <span className="inline-flex items-center gap-1 text-emerald-600 text-xs font-bold bg-emerald-50 px-2 py-1 rounded-full"><CheckCircle size={12}/> ครบถ้วน</span>
                                                 ) : (
                                                     <span className="inline-flex items-center gap-1 text-red-600 text-xs font-bold bg-red-50 px-2 py-1 rounded-full"><AlertCircle size={12}/> ไม่ตรง</span>
                                                 )}
                                             </td>
                                         </tr>
                                     );
                 ...
                 {showOtpModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden relative z-10 animate-in zoom-in-95">
                            <div className="bg-orange-500 p-6 flex justify-between items-start text-white">
                                <div>
                                    <h3 className={`${FONTS.header} text-xl font-bold flex items-center gap-2`}><ShieldCheck/> ยืนยันยอดไม่ตรง (Discrepancy)</h3>
                                    <p className="text-orange-100 text-sm opacity-90">กรุณาขอรหัส OTP จากผู้ขายเพื่อยืนยันการรับสินค้า</p>
                                </div>
                                <button onClick={() => setShowOtpModal(false)}><X/></button>
                            </div>
                            <div className="p-8">
                                <div className="mb-6">
                                    <label className={`${FONTS.header} block text-sm font-bold text-slate-700 mb-2`}>รหัส OTP</label>
                                    <input 
                                        type="text" 
                                        autoFocus
                                        className="w-full text-center text-3xl font-bold tracking-[0.5em] py-4 border-2 border-slate-200 rounded-xl focus:border-orange-500 focus:ring-4 focus:ring-orange-100 outline-none text-slate-800"
                                        placeholder="0000"
                                        maxLength={4}
                                        value={otp}
                                    ...
                                    />
                                     {otpError && (
                                        <p className={`${FONTS.body} text-red-500 text-sm mt-3 text-center flex items-center justify-center gap-1`}>
                                            <AlertCircle size={14} /> {otpError}
                                        </p>
                                    )}
                                </div>
                                <button onClick={handleConfirmOtp} className="w-full py-3.5 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-bold shadow-lg shadow-orange-200 transition">ยืนยัน OTP</button>
                            </div>
                        </div>
                    </div>
                 )}
        ...
        <div className="flex flex-col h-full bg-white relative animate-in fade-in duration-300">
             ...
             {sendingStatus && (
                <div className="absolute inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-3xl p-8 flex flex-col items-center shadow-2xl animate-in zoom-in-95 duration-200 border border-gray-100">
                        <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-4 shadow-lg ${sendingStatus === 'line' ? 'bg-[#06C755] text-white shadow-green-200' : 'bg-blue-500 text-white shadow-blue-200'}`}>
                            {sendingStatus === 'line' ? <MessageCircle size={40} fill="white" /> : <Mail size={40} />}
                        </div>
                        <h3 className={`${FONTS.header} text-2xl font-bold text-gray-800`}>ส่งใบรับสินค้าเรียบร้อยแล้ว</h3>
                        <p className={`${FONTS.body} text-gray-500 mt-2 text-lg`}>
                            {sendingStatus === 'line' ? 'ส่งไปยัง Line ผู้ขายแล้ว' : 'ส่งไปยัง Email ผู้ขายแล้ว'}
                        </p>
                    </div>
                </div>
            )}
                 ...
                 <div className="flex items-center gap-4">
                     ...
                     <div>
                         <h2 className={`${FONTS.header} text-2xl font-bold text-gray-800 flex items-center gap-2`}>
                            <Truck className="text-emerald-600" /> ใบรับสินค้า (Goods Receipt Note)
                         </h2>
                         <p className={`${FONTS.body} text-sm text-gray-400`}>เลขที่: {grData.id} • วันที่: {grData.date}</p>
                     </div>
                 </div>
                 <div className="flex gap-3">
                     <button onClick={() => handleSend('print')} className="flex items-center gap-2 px-6 py-2.5 border border-gray-200 rounded-xl text-gray-600 font-bold hover:bg-gray-50 hover:text-gray-900 transition shadow-sm">
                         <Printer size={18}/> พิมพ์
                     </button>
                     <button onClick={() => handleSend('email')} className="flex items-center gap-2 px-6 py-2.5 bg-blue-50 text-blue-600 border border-blue-100 rounded-xl font-bold hover:bg-blue-100 transition shadow-sm">
                         <Mail size={18}/> อีเมลผู้ขาย
                     </button>
                     <button onClick={() => handleSend('line')} className="flex items-center gap-2 px-6 py-2.5 bg-[#06C755] text-white rounded-xl font-bold hover:bg-[#05b64d] shadow-lg shadow-green-200 transition">
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-gray-50/50">
                <div className="max-w-4xl mx-auto bg-white p-12 rounded-xl shadow-sm border border-gray-100 min-h-[800px]">
                    <div className="flex justify-between items-start mb-12 border-b-2 border-gray-100 pb-8">
                        <div>
                            <h1 className={`${FONTS.header} text-4xl font-bold text-gray-800 mb-2 tracking-tight`}>{SYSTEM_DEFAULTS.shopName}</h1>
                            <p className={`${FONTS.body} text-gray-500 text-sm leading-relaxed`}>123 ถนนสุขุมวิท...<br/>โทร: 02-123-4567</p>
                        </div>
                        <div className="text-right">
                             <div className="inline-block bg-emerald-50 text-emerald-700 px-4 py-1 rounded-lg text-sm font-bold border border-emerald-100 mb-2">ต้นฉบับ</div>
                             <h2 className={`${FONTS.header} text-xl font-bold text-gray-400 uppercase tracking-widest`}>Goods Receipt</h2>
                             <p className={`${FONTS.body} text-gray-800 font-mono text-xl mt-1 font-bold`}>{grData.id}</p>
                        </div>
                    </div>
                    
                    {/* Discrepancy Note */}
                    {inspectionItems.some(i => i.qty !== i.actualQty) && (
                        <div className="mb-8 p-4 bg-orange-50 border border-orange-200 rounded-xl flex items-start gap-3 text-orange-800">
                            <AlertTriangle className="shrink-0 mt-0.5" />
                            <div>
                                <h4 className="font-bold">หมายเหตุ: ยอดรับจริงไม่ตรงกับใบสั่งซื้อ</h4>
                                <p className="text-sm opacity-80">เอกสารนี้ได้รับการยืนยันด้วย OTP จากผู้ขายแล้ว</p>
                            </div>
                        </div>
                    )}
                    ...
                    <table className="w-full mb-12">
                        <thead className="bg-gray-50 border-y border-gray-200">
                            <tr>
                                <th className={`${FONTS.header} py-4 pl-4 text-left text-xs font-bold text-gray-500 w-16`}>ลำดับ</th>
                                <th className={`${FONTS.header} py-4 text-left text-xs font-bold text-gray-500`}>รายการสินค้า</th>
                                <th className={`${FONTS.header} py-4 text-right text-xs font-bold text-gray-500`}>จำนวนสั่ง</th>
                                <th className={`${FONTS.header} py-4 text-right text-xs font-bold text-gray-500`}>รับจริง (หน่วยย่อย)</th>
                                <th className={`${FONTS.header} py-4 pr-4 text-center text-xs font-bold text-gray-500`}>สถานะ</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-100">
                            {inspectionItems.map((item, index) => {
                                const subUnitInfo = getSubUnitInfo(item, item.actualQty);
                                const isMatch = item.qty === item.actualQty;
                                return (
                                    <tr key={item.id}>
                                        <td className="py-5 pl-4 text-center text-gray-400 text-sm">{index + 1}</td>
                                        <td className="py-5">
                                            <p className={`${FONTS.header} text-gray-800 font-bold text-base`}>{item.name}</p>
                                            <p className={`${FONTS.body} text-gray-500 text-xs mt-0.5`}>{item.barcode}</p>
                                        </td>
                                        <td className="py-5 text-right text-gray-500 text-sm">{item.qty} {item.unit}</td>
                                        <td className="py-5 text-right text-gray-800 font-bold text-sm">
                                            {subUnitInfo.display}
                                        </td>
                                        <td className="py-5 pr-4 text-center">
                                             {isMatch ? <CheckCircle className="inline text-emerald-500" size={18}/> : <AlertCircle className="inline text-red-500" size={18}/>}
                                        </td>
                                    </tr>
                                );
                            })}
                    ...
                    <div className="grid grid-cols-2 gap-20 mt-24">
                         <div className="text-center">
                             <div className="border-b border-gray-300 pb-12 mb-3"></div>
                             <p className={`${FONTS.header} text-sm font-bold text-gray-600`}>ผู้รับสินค้า (Receiver)</p>
                             <p className={`${FONTS.body} text-xs text-gray-400 mt-1`}>วันที่ {new Date().toLocaleDateString('th-TH')}</p>
                         </div>
                         <div className="text-center">
                             <div className="border-b border-gray-300 pb-12 mb-3"></div>
                             <p className={`${FONTS.header} text-sm font-bold text-gray-600`}>ผู้ส่งสินค้า (Sender)</p>
                             <p className={`${FONTS.body} text-xs text-gray-400 mt-1`}>วันที่ {new Date().toLocaleDateString('th-TH')}</p>
                         </div>
                    </div>
                </div>
...
const PaymentListView = ({ onProceedToPayment }) => {
    ...
    const filteredInvoices = MOCK_INVOICES.filter(inv => {
        ...
        const matchDate = !filterDate || inv.date === filterDate; // Simple exact match
    ...
    const handleCreateBilling = () => {
        const items = filteredInvoices.filter(inv => selectedInvoices.has(inv.id));
        onProceedToPayment(items);
    };

    const isSupplierActive = filterSupplier !== 'All';
        ...
        <div className="flex flex-col h-full bg-[#F9FAFB] relative">
            ...
            <div className="px-8 py-6 border-b border-gray-200 bg-white">
                ...
                <div className="flex gap-4">
                    <div className="relative">
                         <select 
                            className={`pl-4 pr-10 py-2.5 border rounded-xl text-sm outline-none appearance-none cursor-pointer transition min-w-[200px] ${
                            isSupplierActive 
                            ? 'bg-gray-900 text-white border-gray-900 font-bold' // <-- Active: พื้นดำ ตัวขาว ตัวหนา
                            : 'bg-gray-50 border-gray-200 hover:bg-gray-100 font-medium text-gray-700'
                            }`}
                            value={filterSupplier}
                            onChange={(e) => setFilterSupplier(e.target.value)}
                         >
                            {uniqueSuppliers.map(s => <option key={s} value={s}>{s === 'All' ? 'ผู้จัดจำหน่ายทั้งหมด' : s}</option>)}
                         </select>
                         <ChevronDown className={`absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none ${isSupplierActive ? 'text-white' : 'text-gray-400'}`} size={16} />
                    </div>
                    <div className="relative">
                         <input 
                            type="date"
                            className="pl-10 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-medium outline-none focus:ring-2 focus:ring-emerald-500 cursor-pointer hover:bg-gray-100 transition"
                            value={filterDate}
                ...
                <div className="space-y-4">
                    {filteredInvoices.map(inv => (
                        <div key={inv.id} className={`bg-white p-5 rounded-2xl border transition-all cursor-pointer flex items-center gap-5 group ${selectedInvoices.has(inv.id) ? 'border-emerald-500 bg-emerald-50/10 shadow-sm' : 'border-gray-200 hover:border-emerald-300 hover:shadow-sm'}`} onClick={() => handleSelect(inv.id)}>
                            <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${selectedInvoices.has(inv.id) ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-gray-300 group-hover:border-emerald-300'}`}>
                                <CheckSquare size={14} />
                            </div>
                            <div className="flex-1">
                                <div className="flex justify-between items-start mb-1">
                                    <h4 className={`${FONTS.header} font-bold text-gray-800 text-lg`}>{inv.supplier}</h4>
                                    <span className="text-xs font-bold text-orange-600 bg-orange-50 px-2 py-0.5 rounded-full border border-orange-100">{inv.status}</span>
                                </div>
                                <div className="flex items-center gap-4 text-sm text-gray-500">
                                    <span className="flex items-center gap-1"><FileText size={14}/> {inv.id}</span>
                                    <span className="flex items-center gap-1 text-gray-400">Ref: {inv.grId}</span>
                                    <span className="flex items-center gap-1"><ClockIcon size={14}/> Due: {inv.dueDate}</span>
                                </div>
                            </div>
                            <div className="text-right">
                                <p className={`${FONTS.header} text-xl font-bold text-emerald-600`}>{inv.amount.toLocaleString()}</p>
            ...
            <div className="absolute bottom-0 left-0 right-0 z-30">
                <div className="h-12 bg-gradient-to-t from-[#F9FAFB] to-transparent pointer-events-none"></div>
                <div className="bg-white/90 backdrop-blur-xl border-t border-gray-200 px-8 py-5 shadow-lg flex items-center justify-between">
                    <div>
                        <p className="text-xs text-gray-500 font-bold mb-1">ยอดรวมที่เลือก (Selected Total)</p>
                        <p className={`${FONTS.header} text-3xl font-bold text-emerald-600`}>{SYSTEM_DEFAULTS.currencySymbol}{selectedTotal.toLocaleString()}</p>
                    </div>
                    <button 
                        disabled={selectedInvoices.size === 0}
                        onClick={handleCreateBilling}
                        className={`px-8 py-3.5 rounded-2xl font-bold text-white shadow-xl transition-all transform active:scale-95 flex items-center gap-3 text-lg ${selectedInvoices.size > 0 ? 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-500/20' : 'bg-gray-300 cursor-not-allowed shadow-none'}`}
                    >
                        สร้างใบวางบิล <ArrowLeft className="rotate-180" size={20} strokeWidth={3} />
                    </button>
                </div>
            </div>
        </div>
...
const PaymentProcessView = ({ invoices, onBack, onComplete }) => {
    const [step, setStep] = useState(1); // 1: Billing Note Review, 2: Payment
    const [paymentMethod, setPaymentMethod] = useState('Transfer'); // Transfer, Cheque, CreditCard
    ...
    if (isSuccess) {
        return (
            <div className="flex items-center justify-center h-full bg-[#F9FAFB] animate-in fade-in">
                 <div className="bg-white p-10 rounded-3xl shadow-xl text-center border border-gray-100 flex flex-col items-center">
                     ...
                     <h2 className={`${FONTS.header} text-2xl font-bold text-gray-800 mb-2`}>บันทึกการชำระเงินเรียบร้อย</h2>
                     <p className="text-gray-500">ระบบกำลังนำคุณกลับไปหน้ารายการ...</p>
                 </div>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col h-full bg-[#F9FAFB] relative">
            {/* Header */}
            <div className="px-8 py-5 border-b border-gray-200 bg-white flex items-center gap-4 sticky top-0 z-20">
                <button onClick={onBack} className="p-2 hover:bg-gray-100 rounded-full transition text-gray-500"><ArrowLeft size={24}/></button>
                <div className="flex-1">
                    <h2 className={`${FONTS.header} text-xl font-bold text-gray-800 flex items-center gap-2`}>
                        {step === 1 ? 'สร้างใบวางบิล (Billing Note)' : 'บันทึกการชำระเงิน (Payment)'}
                    </h2>
                    <div className="flex gap-2 mt-1">
                         <div className={`h-1.5 w-8 rounded-full ${step >= 1 ? 'bg-emerald-500' : 'bg-gray-200'}`}></div>
                         <div className={`h-1.5 w-8 rounded-full ${step >= 2 ? 'bg-emerald-500' : 'bg-gray-200'}`}></div>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
                <div className="max-w-4xl mx-auto space-y-6">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
                        <h3 className={`${FONTS.header} font-bold text-gray-800 mb-4 flex items-center gap-2`}><FileText size={20} className="text-emerald-600"/> รายการเอกสาร</h3>
                        <div className="space-y-3">
                            {invoices.map((inv, idx) => (
                                <div key={inv.id} className="flex justify-between items-center text-sm py-2 border-b border-gray-50 last:border-0">
                                    <div className="flex items-center gap-4">
                                        <span className="text-gray-400 w-6 text-center">{idx + 1}</span>
                                        <div>
                                            <p className="font-bold text-gray-700">{inv.id}</p>
                                            <p className="text-gray-400 text-xs">{inv.supplier} • Due: {inv.dueDate}</p>
                                        </div>
                                    </div>
                        ...
                        <div className="mt-6 pt-4 border-t border-gray-100 flex justify-between items-center">
                            <span className="text-gray-500 font-bold">รวมเป็นเงินทั้งสิ้น</span>
                            <span className={`${FONTS.header} text-2xl font-bold text-emerald-600`}>{SYSTEM_DEFAULTS.currencySymbol}{totalAmount.toLocaleString()}</span>
                        </div>
                    </div>

                    {/* Step 2: Payment Form */}
                    {step === 2 && (
                        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 animate-in slide-in-from-bottom-4 duration-300">
                             <h3 className={`${FONTS.header} font-bold text-gray-800 mb-6 flex items-center gap-2`}><CreditCard size={20} className="text-emerald-600"/> ข้อมูลการชำระเงิน</h3>
                             
                             {/* Payment Method Selector */}
                             <div className="grid grid-cols-3 gap-4 mb-6">
                                 {['Transfer', 'Cheque', 'CreditCard'].map(m => (
                                     <button 
                                        key={m}
                                        onClick={() => setPaymentMethod(m)}
                                        className={`py-3 rounded-xl border-2 font-bold text-sm flex flex-col items-center gap-2 transition-all ${paymentMethod === m ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-gray-100 hover:border-gray-300 text-gray-500'}`}
                                     >
                                         {m === 'Transfer' && <Banknote size={24}/>}
                                         {m === 'Cheque' && <FileText size={24}/>}
                                         {m === 'CreditCard' && <CreditCard size={24}/>}
                                         {m === 'Transfer' ? 'เงินโอน' : m === 'Cheque' ? 'เช็ค' : 'บัตรเครดิต'}
                                     </button>
                                 ))}
                             </div>
                             ...
                             <div className="grid grid-cols-2 gap-6">
                                 <div>
                                     <label className="block text-sm font-bold text-gray-700 mb-2">วันที่ชำระ</label>
                                     <input type="date" className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" value={paymentDetails.date} onChange={e => setPaymentDetails({...paymentDetails, date: e.target.value})} />
                                 </div>
                                 <div>
                                     <label className="block text-sm font-bold text-gray-700 mb-2">ธนาคาร / ผู้ให้บริการ</label>
                                     <select className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none" value={paymentDetails.bank} onChange={e => setPaymentDetails({...paymentDetails, bank: e.target.value})}>
                                 ...
                                 <div className="col-span-2">
                                     <label className="block text-sm font-bold text-gray-700 mb-2">
                                         {paymentMethod === 'Transfer' ? 'เลขที่รายการโอน (Ref No.)' : paymentMethod === 'Cheque' ? 'เลขที่เช็ค (Cheque No.)' : 'เลขที่บัตร / Ref (Card No.)'}
                                     </label>
                                     <input 
                                        type="text" 
            ...
            {/* Footer Actions */}
            <div className="bg-white border-t border-gray-200 px-8 py-5 shadow-lg flex justify-end gap-3 sticky bottom-0 z-30">
                 {step === 1 ? (
                     <button onClick={() => setStep(2)} className="px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold shadow-lg shadow-emerald-200 transition transform active:scale-95 flex items-center gap-2">
                         ไปขั้นตอนชำระเงิน <ArrowLeft className="rotate-180" size={20}/>
                     </button>
                 ) : (
                     <button onClick={handleConfirmPayment} className="px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold shadow-lg shadow-emerald-200 transition transform active:scale-95 flex items-center gap-2">
                         <CheckCircle size={20}/> ยืนยันการชำระเงิน
                     </button>
                 )}
            </div>
        </div>
...
// ... PurchaseOrderView, PurchaseOrderDetailView, PurchaseOrderListView, CartView, MarketplaceView, PurchasingModule, App ...
// (I will include the PurchaseOrderDetailView that was missing and ensure other components are correctly defined)

// --- PurchaseOrderDetailView (Restored & Renamed from previous context if needed) ---
const PurchaseOrderDetailView = ({ items, poData, onClose }) => {
    const totalAmount = items.reduce((sum, item) => sum + ((item.cost || 0) * item.qty), 0);
    const poNumber = poData?.id || `PO-${new Date().getFullYear()}${new Date().getMonth() + 1}-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`;
    const today = poData?.date || new Date().toLocaleDateString('th-TH', { year: 'numeric', month: 'long', day: 'numeric' });
    ...
    const getSubUnitInfo = (item) => {
        ...
        return {
            totalSubUnits,
            subUnit,
            display: item.packDetail ? `${item.qty} ${item.unit} (${totalSubUnits} ${subUnit})` : `${item.qty} ${item.unit}`
        };
    };
    ...
    return (
        <div className="flex flex-col h-full bg-white relative animate-in fade-in duration-300">
            ...
            <div className="px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-white sticky top-0 z-20 shadow-sm">
                 <div className="flex items-center gap-4">
                     <button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-full transition text-gray-500"><ArrowLeft size={24}/></button>
                     <div><h2 className={`${FONTS.header} text-2xl font-bold text-gray-800 flex items-center gap-2`}><FileText className="text-emerald-600" /> ใบสั่งซื้อ (Purchase Order)</h2><p className={`${FONTS.body} text-sm text-gray-400`}>เลขที่เอกสาร: {poNumber} • วันที่: {today}</p></div>
                 </div>
                 <div className="flex gap-3">
                     <button onClick={() => handleSend('print')} className="flex items-center gap-2 px-6 py-2.5 border border-gray-200 rounded-xl text-gray-600 font-bold hover:bg-gray-50 hover:text-gray-900 transition shadow-sm"><Printer size={18}/> พิมพ์</button>
                     <button onClick={() => handleSend('email')} className="flex items-center gap-2 px-6 py-2.5 bg-blue-50 text-blue-600 border border-blue-100 rounded-xl font-bold hover:bg-blue-100 transition shadow-sm"><Mail size={18}/> อีเมลผู้ขาย</button>
                     <button onClick={() => handleSend('line')} className="flex items-center gap-2 px-6 py-2.5 bg-[#06C755] text-white rounded-xl font-bold hover:bg-[#05b64d] shadow-lg shadow-green-200 transition"><MessageCircle size={18} fill="white"/> ส่ง Line</button>
                 </div>
            </div>
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-gray-50/50">
                <div className="max-w-4xl mx-auto bg-white p-12 rounded-xl shadow-sm border border-gray-100 min-h-[800px]">
                    <div className="flex justify-between items-start mb-12 border-b-2 border-gray-100 pb-8">
                        <div><h1 className={`${FONTS.header} text-4xl font-bold text-gray-800 mb-2 tracking-tight`}>{SYSTEM_DEFAULTS.shopName}</h1><p className={`${FONTS.body} text-gray-500 text-sm leading-relaxed`}>123 ถนนสุขุมวิท...<br/>โทร: 02-123-4567 | Email: purchase@petomni.com</p></div>
                        <div className="text-right"><div className={`inline-block px-4 py-1 rounded-lg text-sm font-bold border mb-2 ${status === 'Cancelled' ? 'bg-red-50 text-red-700 border-red-100' : 'bg-emerald-50 text-emerald-700 border-emerald-100'}`}>{status === 'New' ? 'ต้นฉบับ (Original)' : status}</div><h2 className={`${FONTS.header} text-xl font-bold text-gray-400 uppercase tracking-widest`}>Purchase Order</h2><p className={`${FONTS.body} text-gray-800 font-mono text-xl mt-1 font-bold`}>{poNumber}</p></div>
                    </div>
                    ...
                    <table className="w-full mb-12">
                        ...
                        <tbody className="divide-y divide-gray-100">{items.map((item, index) => {
                             ...
                             return (
                                <tr key={item.id}>
                                    <td className="py-5 pl-4 text-center text-gray-400 text-sm">{index + 1}</td>
                                    <td className="py-5"><p className={`${FONTS.header} text-gray-800 font-bold text-base`}>{item.name}</p><p className={`${FONTS.body} text-gray-500 text-xs mt-0.5`}>{item.barcode} | {item.brand}</p></td>
                                    <td className="py-5 text-right text-gray-800">
                                        <span className="font-bold text-sm block">{subUnitInfo.display}</span>
                                        {item.packDetail && <span className="text-xs text-gray-400 font-normal">({item.packDetail})</span>}
                                    </td>
                                    <td className="py-5 text-right text-gray-800 text-sm">
                                        {item.cost.toLocaleString()}
                                        <span className="text-gray-400 text-xs block">/{item.unit}</span>
                                    </td>
                                    <td className="py-5 pr-4 text-right text-gray-800 font-bold text-sm">{(item.cost * item.qty).toLocaleString()}</td>
                                </tr>
                            );
                        })}</tbody>
                    </table>
...
const PurchaseOrderListView = ({ onSelectPO, orders = [], onSendPO, onUpdateStatus }) => { 
	...
	// Helper function สำหรับเลือกสีตามสถานะ
	const getStatusColor = (status) => {
		switch (status) {
        ...
        <div className="flex flex-col h-full bg-[#F9FAFB] relative">
             <div className="px-8 py-6 border-b border-gray-200 bg-white shadow-sm z-10">
                    ...
                    <div className="flex gap-2">
                        {['All', 'Pending', 'Received', 'Completed', 'Cancelled'].map(status => (
                            <button
                                key={status}
                                onClick={() => setFilterStatus(status)}
                                className={`px-4 py-2 rounded-lg text-sm transition-all ${filterStatus === status ? 'bg-gray-900 text-white font-bold shadow-md' : 'text-gray-500 font-medium hover:text-gray-900'}`}
                            >
                                {status === 'All' ? 'ทั้งหมด' : status}
                            </button>
                        ))}
                ...
                <div className="relative">
                    ...
                    <input 
                        type="text" 
                        placeholder="ค้นหาเลขที่เอกสาร หรือ ชื่อผู้จัดจำหน่าย..." 
                        className={`${FONTS.header} w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition`}
                        value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                    />
                </div>
                                ...
                                {filteredPOs.length > 0 ? (
                                    filteredPOs.map((po) => (
                                        <tr key={po.id} onClick={() => onSelectPO(po)} className="hover:bg-blue-50/50 transition cursor-pointer group">
                                            {/* ส่วนปุ่ม Action (Email/Line) */}
                                            <td className="p-4 pl-6 text-center" onClick={(e) => e.stopPropagation()}>
                                                {!po.isSent && po.status === 'Pending' ? (
                                                    <div className="flex items-center justify-center gap-2">
                                                        <button 
                                                            onClick={() => onSendPO(po.id, 'Email')}
                                                            className="w-8 h-8 rounded-lg bg-blue-100 text-blue-600 flex items-center justify-center hover:bg-blue-600 hover:text-white transition shadow-sm"
                                                            title="ส่ง Email"
                                                        >
                                                            <Mail size={16} />
                                                        </button>
                                                        <button 
                                                            onClick={() => onSendPO(po.id, 'Line')}
                                                            className="w-8 h-8 rounded-lg bg-green-100 text-green-600 flex items-center justify-center hover:bg-green-600 hover:text-white transition shadow-sm"
                                                            title="ส่ง Line"
                                                        >
                                                    ...
                                                    <div className="flex items-center justify-center text-emerald-500 gap-1 opacity-70">
                                                        <CheckCircle size={16} /> <span className="text-xs font-bold">Sent</span>
                                                    </div>
                                                )}
                                            </td>
                                            ...
                                            <td className="p-4 text-center" onClick={(e) => e.stopPropagation()}>
                                                <div className="relative inline-block">
                                                    <select
                                                        ...
                                                        className={`appearance-none px-3 py-1.5 rounded-full text-xs font-bold border outline-none cursor-pointer text-center min-w-[100px] ${getStatusColor(po.status)}`}
                                                    >
...
const CartView = ({ items, onUpdateQty, onRemove, onSelect, selectedItems, onSelectAll, onCheckout }) => {
    ...
    const getStockDisplay = (item) => {
        ...
        const current = (item.stock || 0) * multiplier;
        const max = (item.stockPolicy?.max || 0) * multiplier;
        const after = ((item.stock || 0) + item.qty) * multiplier;
        
        return { 
            text: `${current.toLocaleString()} / ${max.toLocaleString()} / ${after.toLocaleString()} ${subUnit}`, 
            isOver: after > max 
        };
    };
    ...
    return (
        <div className="flex flex-col h-full w-full bg-[#F3F4F6] relative overflow-hidden">
                ...
                {Object.keys(groupedItems).length > 0 ? Object.entries(groupedItems).map(([supplier, supplierItems]) => {
                    ...
                    return (
                        <div key={supplier} className="bg-white rounded-3xl shadow-sm border border-gray-200/60 overflow-hidden">
                            ...
                            <div className="px-6 py-4 bg-gray-50/80 border-b border-gray-100 flex justify-between items-center backdrop-blur-sm">
                                <div className="flex items-center gap-4">
                                    <button 
                                        onClick={() => { const ids = supplierItems.map(i => i.id); onSelect(ids, !isSupplierAllSelected); }}
                                        className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${isSupplierAllSelected ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-gray-300 hover:border-emerald-300'}`}
                                    >
                                ...
                                <div className="flex items-center gap-2">
                                    <span className={`${FONTS.body} text-xs text-gray-500`}>ชำระโดย:</span>
                                    <select className={`${FONTS.body} text-xs border border-gray-200 rounded-lg px-3 py-1.5 bg-white text-gray-700 outline-none focus:ring-2 focus:ring-emerald-100 focus:border-emerald-400 transition-all cursor-pointer hover:bg-gray-50`}>
                                        <option>โอนเงิน (Bank Transfer)</option>
                                        <option>บัตรเครดิต (Credit Card)</option>
                            ...
                            <div className="divide-y divide-gray-100">
                                {supplierItems.map(item => {
                                    const { text: stockText, isOver } = getStockDisplay(item);
                                        ...
                                        <div key={item.id} className={`p-6 flex gap-6 items-start transition-all duration-200 group ${isSelected ? 'bg-emerald-50/10' : 'hover:bg-gray-50/50'}`}>
                                            <div className="pt-3"> 
                                                <button 
                                                    ...
                                                    className={`w-5 h-5 rounded-md border-2 flex-shrink-0 flex items-center justify-center transition-all ${isSelected ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-gray-300 hover:border-emerald-300'}`}
                                                >
                                                    <Check size={12} strokeWidth={4} />
                                                </button>
                                            </div>
                                            
                                            <div className="w-28 h-28 bg-white rounded-2xl flex items-center justify-center text-5xl flex-shrink-0 border border-gray-100 shadow-sm p-2 relative overflow-hidden group-hover:shadow-md transition-all">
                                                {item.img || '📦'}
                                                {item.tags && item.tags.includes('best_seller') && <div className="absolute top-0 left-0 bg-orange-500 text-white text-[8px] font-bold px-1.5 py-0.5 rounded-br-lg">Best Seller</div>}
                                            </div>
                                            ...
                                            <div className="flex-1 min-w-0 flex flex-col justify-between self-stretch">
                                                ...
                                                <div className="flex justify-between items-start gap-4">
                                                    <div className="space-y-1.5">
                                                        <div className="flex items-center gap-2">
                                                            <span className={`${FONTS.header} text-[10px] font-bold tracking-wider uppercase text-gray-500 bg-gray-100 px-2 py-0.5 rounded-md border border-gray-200`}>{item.brand}</span>
                                                            <span className={`${FONTS.header} text-[10px] font-medium text-gray-400 uppercase tracking-wide`}>{item.category}</span>
                                                        </div>
                                                        <h4 className={`${FONTS.header} font-bold text-gray-800 text-lg leading-tight group-hover:text-emerald-700 transition-colors`}>{item.name}</h4>
                                                        {item.description && <p className={`${FONTS.body} text-xs text-gray-500 line-clamp-1`}>{item.description}</p>}
                                                        <div className="flex items-center gap-2 mt-1">
                                                            {item.packDetail && (
                                                                <span className={`${FONTS.body} text-[10px] font-medium text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100 flex items-center gap-1`}>
                                                                    <Package size={10} /> {item.packDetail}
                                                                </span>
                                                            )}
                                                            <span className={`${FONTS.body} text-[10px] text-gray-400 bg-gray-50 px-2 py-0.5 rounded-full border border-gray-100`}>{item.sku}</span>
                                                        </div>
                                                    </div>
                                                    
                                                    <div className="text-right">
                                                        <p className={`${FONTS.header} text-2xl font-bold text-emerald-600 tracking-tight`}>฿{((item.cost || 0) * item.qty).toLocaleString()}</p>
                                                        <p className={`${FONTS.body} text-xs font-medium text-gray-400`}>@{ (item.cost || 0).toLocaleString() } / {item.unit}</p>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex justify-end items-end mt-4 pt-2">
                                                    <div className="flex items-end gap-5">
                                                        ...
                                                        <div className="flex flex-col items-center gap-2">
                                                            <div className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full flex items-center gap-1.5 transition-colors border ${isOver ? 'bg-red-50 text-red-600 border-red-100' : 'bg-gray-100 text-gray-500 border-gray-200'}`}>
                                                                <span className="tracking-wide">{stockText}</span>
                                                                {isOver && <AlertTriangle size={10} className="animate-pulse" />}
                                                            </div>

                                                            <div className="flex items-center bg-white border border-gray-200 rounded-xl h-11 shadow-sm group-hover:border-emerald-200 transition-all overflow-hidden">
                                                                <button onClick={() => onUpdateQty(item.id, Math.max(1, item.qty - 1))} className="w-11 h-full flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 active:bg-red-100 transition-colors"><Minus size={16}/></button>
                                                                <div className="w-px h-6 bg-gray-100"></div>
                                                                <input 
                                                                    type="text" 
                                                                    value={item.qty} 
                                                                    onChange={(e) => { const val = parseInt(e.target.value); if (!isNaN(val) && val > 0) onUpdateQty(item.id, val); }}
                                                                    className={`${FONTS.header} w-16 text-center font-bold text-gray-800 text-lg outline-none bg-transparent h-full`} 
                                                                />
                                                                <div className="w-px h-6 bg-gray-100"></div>
                                                                <button onClick={() => onUpdateQty(item.id, item.qty + 1)} className="w-11 h-full flex items-center justify-center text-gray-400 hover:text-emerald-600 hover:bg-emerald-50 active:bg-emerald-100 transition-colors"><Plus size={16}/></button>
                                                            </div>
                                                        </div>

                                                        <div className="flex items-center gap-1 pb-1.5 pl-2 border-l border-gray-100">
                                                            <span className={`${FONTS.header} text-sm font-bold text-gray-400 min-w-[30px] text-center`}>{item.unit}</span>
                                                            <button onClick={() => onRemove(item.id)} className="p-2.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all ml-1">
                                                                <Trash2 size={18} />
                                                            </button>
                                                        </div>
                                                    </div>
                    ...
                    <div className="flex flex-col items-center justify-center h-full text-gray-300">
                        <div className="w-32 h-32 bg-gray-100 rounded-full flex items-center justify-center mb-6 animate-pulse">
                            <ShoppingBasket size={64} className="opacity-20 text-gray-500" />
                        </div>
                        <p className={`${FONTS.header} text-xl font-bold text-gray-400`}>ตะกร้าสินค้าว่างเปล่า</p>
                        <p className={`${FONTS.body} text-sm text-gray-400 mt-2`}>เลือกสินค้าจากหน้า Marketplace เพื่อเริ่มสั่งซื้อ</p>
                    </div>
                )}
            </div>
            ...
            <div className="absolute bottom-0 left-0 right-0 z-30">
                ...
                <div className="bg-white/90 backdrop-blur-xl border-t border-gray-200 px-8 py-5 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button 
                            onClick={() => onSelectAll(!allSelected)}
                            className={`w-6 h-6 rounded-md border-2 flex items-center justify-center transition-all ${allSelected ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-gray-300 hover:border-emerald-300'}`}
                        >
                            <Check size={12} strokeWidth={4} />
                        </button>
                        <div>
                            <span className={`${FONTS.header} text-gray-700 font-bold text-base`}>เลือกทั้งหมด</span>
                            <span className={`${FONTS.body} text-xs text-gray-400 ml-2`}>({items.length} รายการ)</span>
                        </div>
                    </div>
                    
                    <div className="flex items-center gap-8">
                        <div className="text-right">
                            <p className={`${FONTS.body} text-xs text-gray-500 font-medium mb-0.5`}>ยอดรวมสุทธิ (Net Total)</p>
                            <div className="flex items-baseline gap-1 justify-end">
                                <span className={`${FONTS.header} text-3xl font-bold text-emerald-600 leading-none tracking-tight`}>฿{totalSelected.toLocaleString()}</span>
                                <span className="text-sm text-gray-400 font-medium">.00</span>
                            </div>
                        </div>
                        <button 
                            onClick={handleInitiateCheckout}
                            disabled={totalSelected === 0}
                            className={`px-10 py-4 rounded-2xl font-bold text-white shadow-xl shadow-emerald-500/20 transition-all transform active:scale-95 flex items-center gap-3 text-lg ${totalSelected > 0 ? 'bg-emerald-600 hover:bg-emerald-700 hover:shadow-emerald-500/40' : 'bg-gray-300 cursor-not-allowed shadow-none'}`}
                        >
                            สั่งซื้อสินค้า <ArrowLeft className="rotate-180" size={22} strokeWidth={3} />
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden relative z-10 animate-in zoom-in-95 duration-200">
                        <div className="bg-slate-900 p-6 flex justify-between items-start">
                            <div>
                                <h3 className={`${FONTS.header} text-xl font-bold text-white mb-1 flex items-center gap-2`}>
                                    <ShieldCheck size={24} className="text-emerald-400" /> ยืนยันคำสั่งซื้อ
                                </h3>
                                <p className={`${FONTS.body} text-slate-400 text-sm`}>กรุณาใส่รหัสผ่านผู้อนุมัติเพื่อดำเนินการต่อ</p>
                            </div>
                            <button onClick={() => setIsApprovalModalOpen(false)} className="text-slate-400 hover:text-white transition"><X size={24} /></button>
                        </div>
                        <div className="p-8">
                            <div className="mb-6">
                                <label className={`${FONTS.header} block text-sm font-bold text-slate-700 mb-2`}>รหัสผ่านผู้อนุมัติ (Approver Password)</label>
                                <div className="relative">
                                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={20} />
                                    <input 
                                        type="password" 
                                        autoFocus
                                        className={`${FONTS.header} w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition text-lg tracking-widest`}
                                        placeholder="••••••"
                                ...
                                {approvalError && (
                                    <p className={`${FONTS.body} text-red-500 text-sm mt-2 flex items-center gap-1 animate-in slide-in-from-top-1`}>
                                        <AlertCircle size={14} /> {approvalError}
                                    </p>
                                )}
                            </div>
                            <div className="flex gap-3">
                                <button 
                                    onClick={() => setIsApprovalModalOpen(false)}
                                    className={`${FONTS.header} flex-1 py-3.5 border border-slate-200 rounded-xl text-slate-600 font-bold hover:bg-slate-50 transition`}
                                >
                                    ยกเลิก
                                </button>
                                <button 
                                    onClick={handleConfirmApproval}
                                    className={`${FONTS.header} flex-1 py-3.5 bg-emerald-600 text-white rounded-xl font-bold hover:bg-emerald-700 shadow-lg shadow-emerald-200 transition transform active:scale-95`}
                                >
                                    ยืนยันอนุมัติ
                                </button>
                            </div>
...
const MarketplaceView = ({ products, addToPurchaseRequest, purchaseRequestItems, onGoToCart }) => {
    ...
    // Local state for quantity input per product (key: product.id, value: quantity)
    ...
    const filteredProducts = products.filter(p => {
        ...
        if (selectedCategory === 'all') {
             ...
             matchesCategory = p.tags && p.tags.includes('best_seller');
        } else if (selectedCategory === 'new') {
             matchesCategory = p.tags && p.tags.includes('new_arrival');
        } else if (['Food', 'Medicine', 'Accessory'].includes(selectedCategory)) {
    ...
    // Helper to get current quantity from cart or local state
    const getProductQty = (productId) => {
        const cartItem = purchaseRequestItems.find(item => item.id === productId);
    ...
    const handleLocalQtyChange = (productId, delta) => {
        ...
        // If item is in cart, update cart directly
        const isInCart = purchaseRequestItems.some(item => item.id === productId);
        if (isInCart) {
            const product = products.find(p => p.id === productId);
            addToPurchaseRequest(product, newQty);
        } else {
            // Just update local state for input display
        ...
        <div className="flex flex-col h-full bg-[#F9FAFB] overflow-y-auto custom-scrollbar">
            ...
            <div className="sticky top-0 z-30 bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between shadow-sm">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={onGoToCart} className="p-2.5 bg-gray-100 hover:bg-gray-200 rounded-xl text-gray-600 transition relative group">
                        <ShoppingCart size={20} className="group-hover:text-emerald-600 transition-colors" />
                        {purchaseRequestItems.length > 0 && (
                            <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-xs rounded-full flex items-center justify-center border-2 border-white animate-in zoom-in font-bold">
                                {purchaseRequestItems.length}
                            </span>
                        )}
                    </button>
                </div>
            ...
            <div className="p-8 space-y-8">
                ...
                <div>
                    ...
                    <div className="flex gap-4 overflow-x-auto pb-2 custom-scrollbar">
                        {MARKETPLACE_CATEGORIES.map(cat => {
                            // Map category ID to logic used in state
                            const catValue = cat.cat || cat.id; // 'Food', 'Medicine', or 'all', 'best'
                            const isActive = selectedCategory === (cat.cat || cat.id);

                            return (
                                <button 
                                    key={cat.id} 
                                    onClick={() => setSelectedCategory(cat.cat || cat.id)}
                                    className={`flex flex-col items-center gap-2 min-w-[100px] p-4 rounded-2xl border transition group ${isActive ? 'bg-emerald-50 border-emerald-500 shadow-md' : 'bg-white border-gray-100 hover:border-emerald-400 hover:shadow-md'}`}
                                >
                                    <span className={`text-3xl transition ${isActive ? 'scale-110' : 'group-hover:scale-110'}`}>{cat.icon}</span>
                                    <span className={`${FONTS.header} text-sm font-bold ${isActive ? 'text-emerald-700' : 'text-gray-600 group-hover:text-emerald-600'}`}>{cat.name}</span>
                                </button>
                            );
                        })}
                    </div>
                ...
                {/* Product Grid */}
                <div>
                    <h3 className={`${FONTS.header} text-lg font-bold text-gray-800 mb-4 flex items-center gap-2`}><TrendingUp size={18} className="text-emerald-600" /> สินค้า{selectedCategory === 'all' ? 'ทั้งหมด' : selectedCategory}</h3>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                        {filteredProducts.map((product) => {
                            ...
                            return (
                                <div key={product.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-xl transition group flex flex-col relative h-[380px]">
                                    {/* Badges */}
                                    <div className="absolute top-3 left-3 z-10 flex flex-col gap-1">
                                        {product.tags && product.tags.includes('best_seller') && (
                                            <span className="bg-orange-500 text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-sm flex items-center gap-1">
                                                <TrendingUp size={10} /> ขายดี
                                            </span>
                                        )}
                                        {product.tags && product.tags.includes('new_arrival') && (
                                            <span className="bg-blue-500 text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-sm flex items-center gap-1">
                                                <Star size={10} /> มาใหม่
                                            </span>
                                        )}
                                    </div>

                                    <div className="h-40 bg-gray-50 flex items-center justify-center text-6xl relative shrink-0">
                                        {product.img}
                                    ...
                                    <div className="p-4 flex-1 flex flex-col">
                                        ...
                                        <h4 className={`${FONTS.header} text-sm font-bold text-gray-800 line-clamp-1 mb-1`}>{product.name}</h4>
                                        {product.description && <p className="text-[10px] text-gray-500 line-clamp-2 mb-2 h-8 leading-tight">{product.description}</p>}
                                        
                                        <div className="mt-auto mb-3">
                                            {/* Price Per Unit */}
                                            <div className="flex items-baseline gap-1">
                                                <span className="text-lg font-bold text-emerald-600">฿{product.cost}</span>
                                                <span className="text-xs text-gray-500 font-medium">/{product.unit}</span>
                                            </div>
                                            {/* Sub-unit / Pack Detail / Minimum Order */}
                                            <p className="text-xs text-gray-400 mt-0.5">
                                                {product.packDetail ? product.packDetail : `ขั้นต่ำ 1 ${product.unit}`}
                                            </p>
                                        </div>

                                        {/* Action Area: Qty Control & Button */}
                                        <div className="flex items-center gap-2">
                                            {/* Quantity Stepper */}
                                            <div className="flex items-center bg-gray-50 rounded-lg border border-gray-200 h-10 flex-1">
                                                <button 
                                                    onClick={() => handleLocalQtyChange(product.id, -1)}
                                                    className="w-8 h-full flex items-center justify-center text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-l-lg transition"
                                                >
                                                    <Minus size={14} />
                                                </button>
                                                <input 
                                                    type="text" 
                                                    className="w-full h-full text-center bg-transparent text-sm font-bold text-gray-800 outline-none"
                                                    value={currentQty}
                                                    onChange={(e) => handleInputChange(product.id, e.target.value)}
                                                />
                                                <button 
                                                    onClick={() => handleLocalQtyChange(product.id, 1)}
                                                    className="w-8 h-full flex items-center justify-center text-gray-400 hover:text-green-500 hover:bg-green-50 rounded-r-lg transition"
                                                >
                                                    <Plus size={14} />
                                                </button>
                                            </div>
                                            ...
                                            {/* Add/Check Button */}
                                            <button 
                                                onClick={() => addToPurchaseRequest(product, currentQty)}
                                                className={`w-10 h-10 rounded-lg flex items-center justify-center transition shadow-sm flex-shrink-0 ${isInCart ? 'bg-emerald-500 text-white hover:bg-emerald-600 shadow-emerald-200' : 'bg-gray-900 text-white hover:bg-black'}`}
                                            >
                                                {isInCart ? <Check size={20} /> : <ShoppingCart size={18} />}
                                            </button>
                                        </div>
                                        
                                        <div className="mt-3 pt-3 border-t border-gray-50 flex items-center gap-2 text-[10px] text-gray-400">
                                            <Truck size={12} /> <span>ส่งฟรีเมื่อสั่งครบ 500.-</span>
                                        </div>
                                    </div>
                                </div>
                            );
...
const CRMSettingsView = () => {
    ...
    const [editingSection, setEditingSection] = useState(null); // 'points', 'levels', 'registration'
    ...
    const [pointConfig, setPointConfig] = useState({
        ...
        points: 1,
        welcomePoints: 0,
        expiry: 'none' // none, 1year, 2years
    });

    const [levelConfig, setLevelConfig] = useState([
        { id: 1, label: 'ผู้พิทักษ์', min: 0, max: 5000, color: 'bg-gray-100 text-gray-600', icon: '🛡️' },
        { id: 2, label: 'คู่หูขนฟู', min: 5001, max: 20000, color: 'bg-blue-100 text-blue-600', icon: '🤝' },
        { id: 3, label: 'ก๊วนเพื่อนซี้', min: 20001, max: 50000, color: 'bg-purple-100 text-purple-600', icon: '✨' },
        { id: 4, label: 'ครอบครัวขนฟู', min: 50001, max: null, color: 'bg-green-100 text-green-600', icon: '🏡' }
    ]);

    const [regConfig, setRegConfig] = useState({
        cardType: 'Virtual Card (แสดงผลบนมือถือ)'
    });
    ...
    const handleLevelChange = (id, field, value) => {
        setLevelConfig(prev => prev.map(l => l.id === id ? { ...l, [field]: value } : l));
    };
    ...
    return (
        <div className="flex-1 overflow-y-auto custom-scrollbar p-8 bg-[#F9FAFB]">
             ...
             <div className="max-w-4xl space-y-6">
                 ...
                 {/* Card 1: Point Exchange (Editable) */}
                 <div className={`bg-white rounded-2xl shadow-sm border ${editingSection === 'points' ? 'border-yellow-400 ring-2 ring-yellow-100' : 'border-gray-100'} p-6 transition-all duration-300`}>
                     <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
                         <h3 className={`${FONTS.header} text-lg font-bold text-gray-800 flex items-center gap-2`}>
                            <Coins size={20} className="text-yellow-500"/> ตั้งค่าการแลกคะแนน
                         </h3>
                         {editingSection === 'points' ? (
                             <div className="flex gap-2">
                                 <button onClick={() => handleCancel('points')} className="text-gray-400 hover:text-gray-600 text-sm font-bold px-3 py-1 bg-gray-50 rounded-lg">ยกเลิก</button>
                                 <button onClick={() => handleSave('points')} className="text-white bg-green-600 hover:bg-green-700 text-sm font-bold px-3 py-1 rounded-lg shadow-sm">บันทึก</button>
                             </div>
                         ) : (
                             <button onClick={() => setEditingSection('points')} className="text-yellow-600 hover:text-yellow-700 font-bold text-sm underline flex items-center gap-1"><Edit2 size={14}/> แก้ไข</button>
                         )}
                     </div>
                     <div className="space-y-4">
                         <div className="flex justify-between items-center py-2 gap-4">
                             <span className="text-gray-500 w-1/3">การแลกแต้ม</span>
                             {editingSection === 'points' ? (
                                 <div className="flex-1 flex items-center gap-2 bg-gray-50 p-2 rounded-lg border border-gray-200">
                                     <span className="text-sm text-gray-600">ทุกๆ</span>
                                     <input 
                                        type="number" 
                                        className="w-20 text-center font-bold border border-gray-300 rounded px-2 py-1 focus:ring-2 focus:ring-yellow-200 outline-none" 
                                        value={pointConfig.spending}
                                        onChange={(e) => setPointConfig({...pointConfig, spending: parseInt(e.target.value) || 0})}
                                     />
                                     <span className="text-sm text-gray-600">บาท = </span>
                                     <input 
                                        type="number" 
                                        className="w-16 text-center font-bold border border-gray-300 rounded px-2 py-1 focus:ring-2 focus:ring-yellow-200 outline-none" 
                                        value={pointConfig.points}
                                        onChange={(e) => setPointConfig({...pointConfig, points: parseInt(e.target.value) || 0})}
                                     />
                                     <span className="text-sm text-gray-600">แต้ม</span>
                                 </div>
                             ) : (
                                 <span className="font-medium text-gray-800 text-right flex-1">สะสมตามยอดทุกๆ {pointConfig.spending} บาท = {pointConfig.points} แต้ม</span>
                             )}
                         </div>
                         <div className="flex justify-between items-center py-2 border-t border-gray-50 gap-4">
                             <span className="text-gray-500 w-1/3">แต้มแรกเข้า</span>
                             {editingSection === 'points' ? (
                                 <div className="flex-1 flex items-center justify-end gap-2">
                                     <input 
                                        type="number" 
                                        className="w-24 text-right font-bold border border-gray-200 rounded px-2 py-1 focus:ring-2 focus:ring-yellow-200 outline-none" 
                                        value={pointConfig.welcomePoints}
                                        onChange={(e) => setPointConfig({...pointConfig, welcomePoints: parseInt(e.target.value) || 0})}
                                     />
                                     <span className="text-sm text-gray-600">แต้ม</span>
                                 </div>
                             ) : (
                                 <span className="font-medium text-gray-800 text-right flex-1">{pointConfig.welcomePoints === 0 ? 'ไม่มี' : `${pointConfig.welcomePoints} แต้ม`}</span>
                             )}
                         </div>
                         <div className="flex justify-between items-center py-2 border-t border-gray-50 gap-4">
                             <span className="text-gray-500 w-1/3">อายุคะแนน</span>
                             {editingSection === 'points' ? (
                                 <select 
                                    className="flex-1 border border-gray-200 rounded px-2 py-1.5 text-right font-bold text-gray-700 bg-white focus:ring-2 focus:ring-yellow-200 outline-none"
                                    value={pointConfig.expiry}
                                    onChange={(e) => setPointConfig({...pointConfig, expiry: e.target.value})}
                                 >
                                     <option value="none">ไม่มีวันหมดอายุ</option>
                                     <option value="1year">1 ปี (นับจากวันที่ได้รับ)</option>
                                     <option value="2years">2 ปี (นับจากวันที่ได้รับ)</option>
                                     <option value="endyear">ตัดรอบทุกสิ้นปี</option>
                                 </select>
                             ) : (
                                 <span className="font-medium text-gray-800 text-right flex-1">
                                     {pointConfig.expiry === 'none' ? 'ไม่มีกำหนด' : 
                                      pointConfig.expiry === '1year' ? '1 ปี' : 
                                      pointConfig.expiry === '2years' ? '2 ปี' : 'ตัดรอบสิ้นปี'}
                                 </span>
                             )}
                         </div>
                     </div>
                 ...
                 {/* Card 2: Member Level (Editable) */}
                 <div className={`bg-white rounded-2xl shadow-sm border ${editingSection === 'levels' ? 'border-yellow-400 ring-2 ring-yellow-100' : 'border-gray-100'} p-6 transition-all duration-300`}>
                     <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
                         <h3 className={`${FONTS.header} text-lg font-bold text-gray-800 flex items-center gap-2`}>
                            <Award size={20} className="text-purple-500"/> ตั้งค่าระดับสมาชิก
                         </h3>
                         {editingSection === 'levels' ? (
                             <div className="flex gap-2">
                                 <button onClick={() => handleCancel('levels')} className="text-gray-400 hover:text-gray-600 text-sm font-bold px-3 py-1 bg-gray-50 rounded-lg">ยกเลิก</button>
                                 <button onClick={() => handleSave('levels')} className="text-white bg-green-600 hover:bg-green-700 text-sm font-bold px-3 py-1 rounded-lg shadow-sm">บันทึก</button>
                             </div>
                         ) : (
                             <button onClick={() => setEditingSection('levels')} className="text-yellow-600 hover:text-yellow-700 font-bold text-sm underline flex items-center gap-1"><Edit2 size={14}/> แก้ไข</button>
                         )}
                     </div>
                     <div className="space-y-4">
                         {levelConfig.map((level, i) => (
                             <div key={level.id} className={`flex items-center gap-4 p-3 rounded-xl border ${editingSection === 'levels' ? 'border-blue-100 bg-blue-50/30' : 'border-gray-50 bg-gray-50/50 hover:bg-white hover:shadow-md'} transition-all duration-200`}>
                                 <div className={`w-12 h-12 rounded-lg ${level.color} flex items-center justify-center text-xl shadow-sm shrink-0`}>{level.icon}</div>
                                 <div className="flex-1 min-w-0">
                                     {editingSection === 'levels' ? (
                                         <div className="flex flex-col gap-2">
                                             <input 
                                                type="text" 
                                                className="font-bold text-gray-800 text-sm border border-gray-300 rounded px-2 py-1 w-full focus:ring-1 focus:ring-blue-300 outline-none" 
                                                value={level.label}
                                             ...
                                             <div className="flex items-center gap-2 text-xs text-gray-500">
                                                 ...
                                                 <input 
                                                    type="number" 
                                                    className="w-20 border border-gray-300 rounded px-1 py-0.5 text-center bg-white" 
                                                    value={level.min}
                                                    onChange={(e) => handleLevelChange(level.id, 'min', parseInt(e.target.value))}
                                                 />
                                                 <span> - {level.max ? level.max.toLocaleString() : 'ขึ้นไป'}</span>
                                             </div>
                                         </div>
                                     ) : (
                                         <>
                                             <p className="font-bold text-gray-800 text-sm">{level.label}</p>
                                             <p className="text-xs text-gray-500">
                                                 ยอดสะสม {level.min.toLocaleString()} - {level.max ? level.max.toLocaleString() : 'ขึ้นไป'} บาท
                                             </p>
                                         </>
                                     )}
                                 </div>
                                 {!editingSection && <ChevronRight size={18} className="text-gray-300" />}
                             </div>
                         ))}
                         ...
                         <div className="flex justify-between items-center py-2 mt-4 pt-4 border-t border-gray-100">
                             ...
                             <span className="font-medium text-gray-800">สิ้นปี (31 ธ.ค.)</span>
                         </div>
                     </div>
                 </div>

                 {/* Card 3: Registration (Editable) */}
                 <div className={`bg-white rounded-2xl shadow-sm border ${editingSection === 'reg' ? 'border-yellow-400 ring-2 ring-yellow-100' : 'border-gray-100'} p-6 transition-all duration-300`}>
                     <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
                         <h3 className={`${FONTS.header} text-lg font-bold text-gray-800 flex items-center gap-2`}>
                             <UserCheck size={20} className="text-green-500"/> ตั้งค่าการสมัครสมาชิก
                         </h3>
                         {editingSection === 'reg' ? (
                             <div className="flex gap-2">
                                 <button onClick={() => handleCancel('reg')} className="text-gray-400 hover:text-gray-600 text-sm font-bold px-3 py-1 bg-gray-50 rounded-lg">ยกเลิก</button>
                                 <button onClick={() => handleSave('reg')} className="text-white bg-green-600 hover:bg-green-700 text-sm font-bold px-3 py-1 rounded-lg shadow-sm">บันทึก</button>
                             </div>
                         ) : (
                             <button onClick={() => setEditingSection('reg')} className="text-yellow-600 hover:text-yellow-700 font-bold text-sm underline flex items-center gap-1"><Edit2 size={14}/> แก้ไข</button>
                         )}
                     </div>
                     <div className="space-y-4">
                         <div className="flex justify-between items-center py-2 gap-4">
                             ...
                             {editingSection === 'reg' ? (
                                 <select 
                                    className="flex-1 border border-gray-200 rounded px-2 py-1.5 text-right font-bold text-gray-700 bg-white focus:ring-2 focus:ring-yellow-200 outline-none"
                                    value={regConfig.cardType}
                                    onChange={(e) => setRegConfig({...regConfig, cardType: e.target.value})}
                                 >
                                     <option>Virtual Card (แสดงผลบนมือถือ)</option>
                                     <option>Physical Card (บัตรแข็ง)</option>
                                     <option>ใช้เบอร์โทรศัพท์เพียงอย่างเดียว</option>
                                 </select>
                             ) : (
                                 <span className="font-medium text-gray-800 text-right">{regConfig.cardType}</span>
                             )}
                         </div>
                     </div>
                 </div>

                 {/* Card 4: Toggle (Simple) */}
                 <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex justify-between items-center group cursor-pointer hover:border-green-200 transition-colors">
                     <div className="flex items-center gap-3">
                        <div className="p-2 bg-green-100 rounded-lg text-green-600"><CheckCircle2 size={20} /></div>
                        <span className="font-bold text-gray-800">เปิดใช้งานระบบสะสมแต้ม</span>
                     </div>
                     <div className="w-12 h-6 bg-green-500 rounded-full p-1 flex justify-end cursor-pointer transition-colors group-hover:bg-green-600">
                         <div className="w-4 h-4 bg-white rounded-full shadow-sm"></div>
                     </div>
                 </div>
             </div>

             <button className="fixed bottom-8 right-8 w-14 h-14 bg-[#FCD34D] rounded-full shadow-lg flex items-center justify-center hover:scale-105 transition text-gray-900 z-50">
                <Home size={24} />
             </button>
        </div>
    );
...
const MemberDetailView = ({ member, onBack }) => {
    ...
    const [editingSection, setEditingSection] = useState(null); // 'profile', 'personal', 'address', 'stats'
    const [activePetId, setActivePetId] = useState(member.petIds?.[0] || null);
    const [panelMode, setPanelMode] = useState(member.petIds?.[0] ? 'detail' : null); // 'detail', 'add', 'edit', 'history_grooming'
    const [petStatCounts, setPetStatCounts] = useState({});
    
    // Pet Add/Edit Form State
    ...
    // Handle Pet Form Populate
    useEffect(() => {
        if (panelMode === 'edit' && activePet) {
            setPetForm({
                ...activePet,
                // Map existing gender to new options if necessary, or keep as is
                gender: activePet.gender || 'Male',
    ...
    // Handle Address Editing Populating
    useEffect(() => {
    ...
    const handleStatChange = (type, delta) => {
        setPetStatCounts(prev => ({
            ...prev,
    ...
    const handlePetTypeChange = (e) => {
        setPetForm({ ...petForm, type: e.target.value, breed: '' }); // Reset breed on type change
        setBreedSuggestions([]);
    };

    const handleBreedChange = (e) => {
        const val = e.target.value;
        setPetForm({ ...petForm, breed: val });
        
        if (val.length > 0) {
            const breeds = ANIMAL_BREEDS_DATA[petForm.type] || [];
            const filtered = breeds.filter(b => b.toLowerCase().includes(val.toLowerCase()));
    ...
    const selectBreed = (breed) => {
        setPetForm({ ...petForm, breed: breed });
        setShowBreedSuggestions(false);
    };
    ...
    const handleSubDistrictChange = (sub) => {
        const matched = THAI_ADDRESS_MOCK.find(a => 
            a.province === addressForm.province && 
            a.district === addressForm.district && 
            a.subDistrict.includes(sub)
        );
    ...
    const PET_TYPE_CONFIG = [
        ...
        { id: 'rabbit', label: 'กระต่าย', icon: '🐰' },
        { id: 'rodent', label: 'สัตว์ฟันแทะ', icon: '🐹' },
        { id: 'reptile', label: 'สัตว์เลื้อยคลาน', icon: '🦎' },
        { id: 'bird', label: 'นก', icon: '🦜' },
        { id: 'fish', label: 'ปลา', icon: '🐟' },
    ];
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-[#F9FAFB] h-full overflow-hidden">
            ...
            <header className="h-16 bg-white border-b border-gray-100 px-6 flex items-center justify-between shrink-0 sticky top-0 z-10">
                <div className="flex items-center gap-4">
                    ...
                    <h1 className={`${FONTS.header} text-xl font-bold text-gray-800`}>ข้อมูลสมาชิก</h1>
                </div>
            </header>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-8">
                <div className="max-w-5xl mx-auto space-y-6">
                    ...
                    {/* Zone 1: Profile Header (Editable) */}
                    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex justify-between items-start group relative">
                        <div className="flex gap-6 w-full">
                            <div className={`w-24 h-24 rounded-full flex items-center justify-center text-4xl text-white font-bold shadow-lg ${localMember.theme === 'amber' ? 'bg-amber-400' : localMember.theme === 'purple' ? 'bg-purple-500' : localMember.theme === 'blue' ? 'bg-blue-500' : localMember.theme === 'pink' ? 'bg-pink-500' : 'bg-indigo-500'}`}>
                                {localMember.name.split(' ')[1]?.charAt(0) || 'A'}
                            </div>
                            <div className="flex-1">
                                <div className="flex items-center gap-3 mb-1">
                                    {editingSection === 'profile' ? (
                                        <div className="flex items-center gap-2 w-full max-w-md">
                                            <input 
                                                type="text" 
                                                className="flex-1 border border-gray-300 rounded-lg px-3 py-1 text-lg font-bold text-gray-900 focus:ring-2 focus:ring-blue-200 outline-none"
                                                value={localMember.name}
                                                onChange={e => setLocalMember({...localMember, name: e.target.value})}
                                            />
                                            <button onClick={() => handleSaveSection('profile')} className="p-2 bg-green-100 text-green-600 rounded-lg hover:bg-green-200"><Save size={18}/></button>
                                            <button onClick={() => setEditingSection(null)} className="p-2 bg-gray-100 text-gray-500 rounded-lg hover:bg-gray-200"><X size={18}/></button>
                                        </div>
                                    ) : (
                                        <>
                                            <h2 className={`${FONTS.header} text-2xl font-bold text-gray-900`}>{localMember.name}</h2>
                                            <button 
                                                onClick={() => setEditingSection('profile')}
                                                className="text-gray-300 hover:text-gray-500 transition p-1 rounded-full hover:bg-gray-50"
                                            >
                                                <Edit2 size={16} />
                                            </button>
                                        </>
                                ...
                                <span className={`${FONTS.header} px-3 py-1 rounded-full text-xs font-bold ${localMember.levelColor}`}>
                                    {localMember.level}
                                </span>
                            </div>
                        </div>
                        <div className="text-right">
                            ...
                            <p className={`${FONTS.header} text-3xl font-bold text-[#F59E0B]`}>{localMember.points.toLocaleString()}</p>
                        </div>
                    </div>

                    {/* Zone 2: Personal Info & Address (Editable) */}
                    <div className="grid grid-cols-2 gap-6">
                        {/* Personal Info */}
                        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 relative">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className={`${FONTS.header} font-bold text-gray-800 flex items-center gap-2`}><User size={18} className="text-gray-400"/> ข้อมูลส่วนตัว</h3>
                                {editingSection === 'personal' ? (
                                    <div className="flex gap-2">
                                        <button onClick={() => handleSaveSection('personal')} className="text-green-600 hover:text-green-700 bg-green-50 p-1.5 rounded-lg"><Save size={16} /></button>
                                        <button onClick={() => setEditingSection(null)} className="text-gray-400 hover:text-gray-600 bg-gray-50 p-1.5 rounded-lg"><X size={16} /></button>
                                    </div>
                                ) : (
                                    <button onClick={() => setEditingSection('personal')} className="text-gray-400 hover:text-gray-600 transition"><Edit2 size={16} /></button>
                                )}
                            </div>
                            <div className="space-y-3">
                                {editingSection === 'personal' ? (
                                    <div className="space-y-3">
                                        <div className="grid grid-cols-3 gap-2 text-sm items-center">
                                            <span className="text-gray-400">ชื่อเล่น</span>
                                            <input type="text" className="col-span-2 border border-gray-200 rounded px-2 py-1" value={localMember.nickname} onChange={e => setLocalMember({...localMember, nickname: e.target.value})} />
                                        </div>
                                        <div className="grid grid-cols-3 gap-2 text-sm items-center">
                                            <span className="text-gray-400">วันเกิด</span>
                                            <input type="text" className="col-span-2 border border-gray-200 rounded px-2 py-1" value={localMember.dob} onChange={e => setLocalMember({...localMember, dob: e.target.value})} />
                                        </div>
                                        <div className="grid grid-cols-3 gap-2 text-sm items-center">
                                            <span className="text-gray-400">เบอร์โทร</span>
                                            <input type="text" className="col-span-2 border border-gray-200 rounded px-2 py-1" value={localMember.phone} onChange={e => setLocalMember({...localMember, phone: e.target.value})} />
                                        </div>
                                    </div>
                                ) : (
                                    <div className="space-y-3">
                                        <div className="grid grid-cols-3 gap-2 text-sm">
                                            <span className="text-gray-400">ชื่อเล่น</span>
                                            <span className="col-span-2 font-medium text-gray-700">{localMember.nickname || '-'}</span>
                                        </div>
                                        <div className="grid grid-cols-3 gap-2 text-sm">
                                            <span className="text-gray-400">วันเกิด</span>
                                            <span className="col-span-2 font-medium text-gray-700">{localMember.dob || '-'}</span>
                                        </div>
                                        <div className="grid grid-cols-3 gap-2 text-sm">
                                            <span className="text-gray-400">เบอร์โทร</span>
                                            <span className="col-span-2 font-medium text-gray-700">{localMember.phone}</span>
                                        </div>
                                        <div className="grid grid-cols-3 gap-2 text-sm items-center">
                                            <span className="text-gray-400">Line ID</span>
                                            {localMember.line ? (
                                                <button className="col-span-2 flex items-center gap-2 text-green-600 font-bold bg-green-50 px-3 py-1.5 rounded-lg hover:bg-green-100 transition w-fit border border-green-200 shadow-sm">
                                                    <MessageCircle size={16} fill="currentColor" /> ติดต่อผ่านไลน์
                                                </button>
                                            ) : (
                                                <span className="col-span-2 font-medium text-gray-400">-</span>
                                            )}
                                        </div>
                                    </div>
                                )}
                        ...
                        <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 relative">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className={`${FONTS.header} font-bold text-gray-800 flex items-center gap-2`}><MapPin size={18} className="text-gray-400"/> ที่อยู่</h3>
                                {editingSection === 'address' ? (
                                    <div className="flex gap-2">
                                        <button onClick={() => handleSaveSection('address')} className="text-green-600 hover:text-green-700 bg-green-50 p-1.5 rounded-lg"><Save size={16} /></button>
                                        <button onClick={() => setEditingSection(null)} className="text-gray-400 hover:text-gray-600 bg-gray-50 p-1.5 rounded-lg"><X size={16} /></button>
                                    </div>
                                ) : (
                                    <button onClick={() => setEditingSection('address')} className="text-gray-400 hover:text-gray-600 transition"><Edit2 size={16} /></button>
                                )}
                            </div>
                            <div className="space-y-4">
                                {editingSection === 'address' ? (
                                    <div className="space-y-3">
                                            ...
                                            {showPostcodeSuggestions && postcodeSuggestions.length > 0 && (
                                                <ul className="absolute z-50 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-40 overflow-y-auto">
                                                    {postcodeSuggestions.map((item, idx) => (
                                                        <li 
                                                            key={idx}
                                                            onClick={() => selectPostcode(item)}
                                                            className="p-2 text-xs hover:bg-blue-50 cursor-pointer border-b border-gray-50 last:border-0"
                                                        >
                                                            <span className="font-bold text-blue-600">{item.postcode}</span> - {item.district} - {item.province}
                                                        </li>
                                                    ))}
                                                </ul>
                                            )}
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                             <div>
                                                ...
                                                <select
                                                    ...
                                                    onChange={e => setAddressForm({...addressForm, province: e.target.value, district: '', subDistrict: ''})}
                                                >
                                                    <option value="">เลือกจังหวัด</option>
                                                    {getFilteredProvinces().map(p => <option key={p} value={p}>{p}</option>)}
                                                </select>
                                             </div>
                                             <div>
                                                ...
                                                <select 
                                                    ...
                                                    onChange={e => setAddressForm({...addressForm, district: e.target.value, subDistrict: ''})}
                                        ...
                                        <div>
                                            <label className="text-xs text-gray-400 mb-1 block">แขวง/ตำบล</label>
                                            <select 
                                                className="w-full border border-gray-200 rounded-lg p-2 text-sm"
                                            ...
                                            >
                                                ...
                                                {getFilteredSubDistricts().map(s => <option key={s} value={s}>{s}</option>)}
                                            </select>
                                        </div>
                                        <div>
                                            <label className="text-xs text-gray-400 mb-1 block">รายละเอียดที่อยู่ (บ้านเลขที่, ถนน, ซอย)</label>
                                            <textarea 
                                                className="w-full border border-gray-200 rounded-lg p-2 text-sm"
                                                rows="2"
                                    ...
                                    <div>
                                        <p className="text-xs text-gray-400 mb-1">ที่อยู่ติดต่อ / จัดส่ง</p>
                                        <p className="text-sm text-gray-700 font-medium leading-relaxed">{localMember.address || '-'}</p>
                                    </div>
                                )}
                                
                                {editingSection !== 'address' && (
                                    <div className="h-24 bg-gray-100 rounded-xl flex items-center justify-center border border-gray-200">
                                        <p className="text-xs text-gray-400 flex items-center gap-1"><MapIcon size={14}/> แผนที่ (Mockup)</p>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {/* Zone 3: Pet Stats (Editable) */}
                    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 relative">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className={`${FONTS.header} font-bold text-gray-800 flex items-center gap-2`}><PawPrint size={18} className="text-gray-400"/> จำนวนสัตว์เลี้ยง (ทั้งหมด)</h3>
                            {editingSection === 'stats' ? (
                                <div className="flex gap-2">
                                    <button onClick={() => handleSaveSection('stats')} className="text-green-600 hover:text-green-700 bg-green-50 p-1.5 rounded-lg"><Save size={16} /></button>
                                    <button onClick={() => setEditingSection(null)} className="text-gray-400 hover:text-gray-600 bg-gray-50 p-1.5 rounded-lg"><X size={16} /></button>
                                </div>
                            ) : (
                                <button onClick={() => setEditingSection('stats')} className="text-gray-400 hover:text-gray-600 transition"><Edit2 size={16} /></button>
                            )}
                        </div>
                        
                        {editingSection === 'stats' ? (
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 animate-in fade-in zoom-in duration-200">
                                {PET_TYPE_CONFIG.map(type => (
                                    <div key={type.id} className="bg-gray-50 border border-gray-200 rounded-xl p-3 flex flex-col items-center gap-2">
                                        <div className="flex items-center gap-2">
                                            <span className="text-xl">{type.icon}</span>
                                            <span className="text-xs font-bold text-gray-600">{type.label}</span>
                                        </div>
                                        <div className="flex items-center bg-white rounded-lg shadow-sm border border-gray-200">
                                            <button onClick={() => handleStatChange(type.id, -1)} className="p-2 text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-l-lg"><Minus size={14}/></button>
                                            <input 
                                                type="number" 
                                                className="w-12 text-center text-sm font-bold text-gray-800 outline-none" 
                                                value={petStatCounts[type.id] || 0}
                                                onChange={(e) => {
                                                    const val = parseInt(e.target.value) || 0;
                                                    setPetStatCounts({...petStatCounts, [type.id]: val});
                                                }}
                                            />
                                            <button onClick={() => handleStatChange(type.id, 1)} className="p-2 text-green-500 hover:text-green-700 hover:bg-green-50 rounded-r-lg"><Plus size={14}/></button>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="flex gap-4 flex-wrap">
                                {Object.keys(petStatCounts).length > 0 && Object.values(petStatCounts).some(v => v > 0) ? 
                                    Object.entries(petStatCounts).filter(([_, count]) => count > 0).map(([typeId, count]) => {
                                        const typeConfig = PET_TYPE_CONFIG.find(t => t.id === typeId) || { icon: '🐾', label: typeId };
                                        return (
                                            <div key={typeId} className="flex items-center gap-3 px-4 py-3 bg-gray-50 rounded-xl border border-gray-100 min-w-[120px]">
                                                <div className="text-2xl">{typeConfig.icon}</div>
                                                <div>
                                                    <p className="text-xs text-gray-400 uppercase font-bold">{typeConfig.label}</p>
                                                    <p className="text-lg font-bold text-gray-800">{count} ตัว</p>
                                                </div>
                                            </div>
                                        );
                                    }) : (
                                        <p className="text-sm text-gray-400">ยังไม่มีข้อมูลจำนวนสัตว์เลี้ยง</p>
                                    )
                                }
                            </div>
                        )}
                    ...
                    {/* Zone 4: History (Bills & Products) */}
                    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                        <div className="flex border-b border-gray-100">
                            <button className="px-6 py-4 text-sm font-bold text-gray-800 border-b-2 border-gray-800">ประวัติการซื้อ (บิล)</button>
                            <button className="px-6 py-4 text-sm font-bold text-gray-400 hover:text-gray-600 transition">ประวัติสินค้า</button>
                        </div>
                        <div className="p-0">
                            {billHistory.length > 0 ? (
                                <table className="w-full text-left text-sm">
                                    <thead className="bg-gray-50 text-gray-500">
                                        <tr>
                                            <th className="p-4 pl-6">เลขที่บิล</th>
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {billHistory.map(bill => (
                                            <tr key={bill.id} className="hover:bg-gray-50">
                                                <td className="p-4 pl-6 font-mono text-blue-600">{bill.id}</td>
                                                <td className="p-4 text-gray-600">{bill.date}</td>
                                                <td className="p-4 text-center">{bill.items}</td>
                                                <td className="p-4 text-right font-bold">{bill.total.toLocaleString()}</td>
                                                <td className="p-4 text-center"><span className="px-2 py-1 bg-green-100 text-green-700 rounded text-xs font-bold">{bill.status}</span></td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            ) : (
                                <div className="p-8 text-center text-gray-400">ไม่พบประวัติการซื้อ</div>
                            )}
                        </div>
                    </div>

                    {/* Zone 5: Pet Management (Split View) */}
                    <div className="grid grid-cols-12 gap-6 min-h-[500px]">
                        ...
                        <div className="col-span-4 bg-white rounded-3xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
                            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                                <h3 className={`${FONTS.header} font-bold text-gray-800`}>สัตว์เลี้ยง</h3>
                                <button 
                                    onClick={handleAddPet}
                                    className="w-8 h-8 bg-gray-900 rounded-full flex items-center justify-center text-white hover:scale-105 transition"
                                >
                                    <Plus size={16}/>
                                </button>
                            </div>
                            <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-2">
                                {memberPets.map(pet => {
                                    const hasAppointment = INITIAL_APPOINTMENTS.some(appt => appt.petName === pet.name && (appt.status === 'Confirmed' || appt.status === 'Checked-in'));
                                    ...
                                    return (
                                        <div 
                                            ...
                                            className={`p-3 rounded-xl cursor-pointer transition flex items-center gap-3 border relative ${activePetId === pet.id && panelMode === 'detail' ? 'bg-blue-50 border-blue-200 shadow-sm' : 'bg-white border-transparent hover:bg-gray-50 hover:border-gray-200'}`}
                                        >
                                            <div className={`w-12 h-12 rounded-full flex items-center justify-center text-2xl bg-white border ${activePetId === pet.id ? 'border-blue-100' : 'border-gray-100'}`}>
                                                {pet.type === 'dog' ? '🐶' : pet.type === 'cat' ? '🐱' : '🐾'}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <h4 className={`font-bold text-sm truncate ${activePetId === pet.id && panelMode === 'detail' ? 'text-blue-700' : 'text-gray-700'}`}>{pet.name}</h4>
                                                <p className="text-xs text-gray-500 truncate">{pet.breed}</p>
                                            </div>
                                            <div className="absolute top-2 right-2 flex gap-1">
                                                {hasAppointment && <Clock3 size={14} className="text-purple-500 fill-purple-100" />}
                                                {hasDisease && <AlertCircle size={14} className="text-red-500 fill-red-50" />}
                                            </div>
                                        </div>
                                    );
                                })}
                        ...
                        <div className="col-span-8 bg-white rounded-3xl shadow-sm border border-gray-100 p-8 flex flex-col">
                            {panelMode === 'detail' && activePet ? (
                                <>
                                    <div className="flex justify-between items-start mb-8">
                                        <div className="flex gap-6 items-center">
                                            <div className="w-24 h-24 rounded-full bg-gray-50 border-4 border-white shadow-md flex items-center justify-center text-5xl relative">
                                                {activePet.type === 'dog' ? '🐶' : activePet.type === 'cat' ? '🐱' : '🐾'}
                                            </div>
                                            <div>
                                                <h2 className={`${FONTS.header} text-3xl font-bold text-gray-800 mb-1`}>{activePet.name}</h2>
                                                <div className="flex items-center gap-3 text-sm text-gray-500">
                                                    <span className="px-3 py-1 bg-gray-100 rounded-full font-bold">{activePet.breed}</span>
                                                    <span>{activePet.gender === 'Male' ? '♂ ชาย' : activePet.gender === 'Female' ? '♀ หญิง' : activePet.gender === 'Male (Neutered)' ? '♂ ชาย (ตอนแล้ว)' : '♀ หญิง (ตอนแล้ว)'}</span>
                                                </div>
                                            </div>
                                        </div>
                                        <button 
                                            onClick={handleEditPet}
                                            className="flex items-center gap-2 text-gray-400 hover:text-gray-600 transition px-3 py-2 rounded-lg hover:bg-gray-50"
                                        >
                                            <Edit2 size={16} /> <span className="text-sm font-bold">แก้ไข</span>
                                        </button>
                                    </div>

                                    <div className="grid grid-cols-2 gap-8 mb-8">
                                        <div className="space-y-1">
                                            <p className="text-xs text-gray-400 font-bold uppercase">วันเกิด / อายุ</p>
                                            <p className="text-lg font-medium text-gray-700">{activePet.birthYear} ({new Date().getFullYear() - activePet.birthYear} ปี)</p>
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-xs text-gray-400 font-bold uppercase">น้ำหนักล่าสุด</p>
                                            <p className="text-lg font-medium text-gray-700">{activePet.weight} กก.</p>
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-xs text-gray-400 font-bold uppercase">ลักษณะขน</p>
                                            <p className="text-lg font-medium text-gray-700">{activePet.coatType || '-'}</p>
                                        </div>
                                        <div className="space-y-1 col-span-2">
                                            <p className="text-xs text-gray-400 font-bold uppercase mb-2">โรคประจำตัว / ข้อควรระวัง</p>
                                            <div className="flex flex-wrap gap-2">
                                                {activePet.chronicDiseases.length > 0 ? (
                                                    activePet.chronicDiseases.map((d, i) => (
                                                    ...
                                                    <span className="text-gray-400 text-sm">- ปลอดภัย แข็งแรง -</span>
                                                )}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-auto pt-6 border-t border-gray-100 grid grid-cols-2 gap-3">
                                        <button className="py-3 bg-blue-50 text-blue-600 rounded-xl font-bold hover:bg-blue-100 transition flex items-center justify-center gap-2">
                                            <History size={18}/> ประวัติการรักษา
                                        </button>
                                        <button 
                                            onClick={() => setPanelMode('history_grooming')}
                                            className="py-3 bg-orange-50 text-orange-600 rounded-xl font-bold hover:bg-orange-100 transition flex items-center justify-center gap-2"
                                        >
                                            <Scissors size={18}/> ประวัติอาบน้ำตัดขน
                                        </button>
                                    </div>
                                </>
                                ...
                                <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-300">
                                    <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
                                        <h3 className={`${FONTS.header} text-xl font-bold text-gray-800`}>
                                            {panelMode === 'add' ? 'เพิ่มสัตว์เลี้ยงใหม่' : 'แก้ไขข้อมูลสัตว์เลี้ยง'}
                                        </h3>
                                        <button onClick={() => { setActivePetId(member.petIds[0] || null); setPanelMode(member.petIds[0] ? 'detail' : null); }} className="text-gray-400 hover:text-gray-600"><X size={20}/></button>
                                    </div>
                                    <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 pr-2">
                                        <div className="flex justify-center mb-4">
                                            <div className="w-24 h-24 rounded-full bg-gray-100 border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-400 hover:bg-gray-50 cursor-pointer transition">
                                                <ImageIcon size={24} />
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="text-sm text-gray-500 block mb-1">ชื่อน้อง</label>
                                                <input 
                                                    type="text" 
                                            ...
                                            <div>
                                                <label className="text-sm text-gray-500 block mb-1">เพศ</label>
                                                <select 
                                                ...
                                                >
                                                    ...
                                                    <option value="Male (Neutered)">เพศผู้ ตอนแล้ว (Male Neutered)</option>
                                                    <option value="Female">เพศเมีย (Female)</option>
                                                    <option value="Female (Neutered)">เพศเมีย ตอนแล้ว (Female Neutered)</option>
                                                </select>
                                            </div>

                                            {/* --- UPDATED: Type Selection --- */}
                                            <div>
                                                <label className="text-sm text-gray-500 block mb-1">ชนิดสัตว์เลี้ยง</label>
                                                <select 
                                                    className="w-full border border-gray-200 rounded-lg p-2 bg-white"
                                                    value={petForm.type}
                                                ...
                                                >
                                                    ...
                                                    <option value="rodent">สัตว์ฟันแทะ (Rodent)</option>
                                                    <option value="reptile">สัตว์เลื้อยคลาน (Reptile)</option>
                                                </select>
                                            </div>

                                            {/* --- UPDATED: Breed with Autocomplete --- */}
                                            <div className="relative">
                                                <label className="text-sm text-gray-500 block mb-1">สายพันธุ์</label>
                                                <input 
                                                    type="text" 
                                                    className="w-full border border-gray-200 rounded-lg p-2" 
                                                    value={petForm.breed}
                                                    onChange={handleBreedChange}
                                                    placeholder="พิมพ์เพื่อค้นหาสายพันธุ์..."
                                                />
                                                    ...
                                                    <ul className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-40 overflow-y-auto">
                                                        {breedSuggestions.map((breed, idx) => (
                                                            <li 
                                                                ...
                                                                className="p-2 text-sm hover:bg-gray-50 cursor-pointer border-b border-gray-50 last:border-0"
                                                            >
                                                                {breed}
                                                            </li>
                                                        ))}
                                            ...
                                            <div>
                                                <label className="text-sm text-gray-500 block mb-1">ปีเกิด</label>
                                                <input 
                                                    type="number" 
                                                    className="w-full border border-gray-200 rounded-lg p-2" 
                                                    value={petForm.birthYear}
                                                    onChange={(e) => setPetForm({...petForm, birthYear: e.target.value})}
                                                />
                                            </div>
                                            <div>
                                                <label className="text-sm text-gray-500 block mb-1">น้ำหนัก (กก.)</label>
                                                <input 
                                                    type="number" 
                                                    className="w-full border border-gray-200 rounded-lg p-2" 
                                            ...
                                            <div className="col-span-2">
                                                <label className="text-sm text-gray-500 block mb-1">ลักษณะขน</label>
                                                <select 
                                                    className="w-full border border-gray-200 rounded-lg p-2 bg-white" 
                                                ...
                                                >
                                                    ...
                                                    <option value="ขนสั้น 2 ชั้น">ขนสั้น 2 ชั้น</option>
                                                    <option value="ขนยาว">ขนยาว</option>
                                                    <option value="ขนยาว 2 ชั้น">ขนยาว 2 ชั้น</option>
                                                    <option value="ไร้ขน">ไร้ขน</option>
                                                    <option value="ขนหยิก">ขนหยิก</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="pt-4 border-t border-gray-100 flex gap-3">
                                        <button onClick={() => { setActivePetId(member.petIds[0] || null); setPanelMode(member.petIds[0] ? 'detail' : null); }} className="flex-1 py-2 border border-gray-200 rounded-lg text-gray-600 font-bold hover:bg-gray-50">ยกเลิก</button>
                                        <button className="flex-1 py-2 bg-emerald-600 text-white rounded-lg font-bold hover:bg-emerald-700 shadow-lg shadow-emerald-100">บันทึก</button>
                                    </div>
                                </div>
                            ) : panelMode === 'history_grooming' ? (
                                <div className="flex flex-col h-full animate-in fade-in slide-in-from-right-4 duration-300">
                                    <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
                                        <h3 className={`${FONTS.header} text-xl font-bold text-gray-800 flex items-center gap-2`}>
                                            <Scissors size={20} className="text-orange-500"/> ประวัติอาบน้ำตัดขน
                                        </h3>
                                        <button onClick={() => setPanelMode('detail')} className="text-gray-400 hover:text-gray-600"><X size={20}/></button>
                                    </div>
                                    <div className="flex-1 overflow-y-auto custom-scrollbar">
                                        <table className="w-full text-left text-sm">
                                            ...
                                            <tbody className="divide-y divide-gray-100">
                                                {MOCK_GROOMING_HISTORY.map((h) => (
                                                    <tr key={h.id} className="hover:bg-gray-50">
                                                        <td className="p-3 pl-4 text-gray-600">{h.date}</td>
                                                        <td className="p-3 font-bold text-gray-700">
                                                            {h.service}
                                                            {h.note && h.note !== '-' && <div className="text-xs text-gray-400 font-normal mt-0.5">Note: {h.note}</div>}
                                                        </td>
                                                        <td className="p-3 text-gray-500">{h.branch}</td>
                                                        <td className="p-3 text-right font-bold text-emerald-600">{h.amount}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                                    ...
                                    <div className="pt-4 border-t border-gray-100">
                                        <button onClick={() => setPanelMode('detail')} className="w-full py-2 border border-gray-200 rounded-lg text-gray-600 font-bold hover:bg-gray-50">กลับหน้ารายละเอียด</button>
                                    </div>
                                </div>
                            ) : (
                                <div className="h-full flex flex-col items-center justify-center text-gray-300">
                                    <PawPrint size={64} className="mb-4 opacity-20"/>
                                    <p>เลือกสัตว์เลี้ยงเพื่อดูรายละเอียด</p>
                                </div>
                            )}
                        </div>
                    </div>
...
const CRMModule = ({ 
    ...
    // NEW: State for Member Detail View
    ...
    const filteredPets = useMemo(() => {
        return PetsDB.filter(pet => {
            ...
            // ... (Pet filtering logic remains same) ...
            let matchesAge = true;
    ...
    if (viewMember) {
        return <MemberDetailView member={viewMember} onBack={() => setViewMember(null)} />;
    }

    return (
		<div className="h-screen w-full bg-[#F9FAFB] flex overflow-hidden">
             {/* ✅ CRMSidebar จะแสดงอยู่ตลอดเวลาในทุก Tab */}
             <CRMSidebar onOpenDrawer={() => setIsDrawerOpen(true)} onNavigate={handleNavigate} activeTab={activeTab} onTabChange={setActiveTab} />
            
            {/* ส่วนแสดงผลด้านขวา เปลี่ยนตาม activeTab */}
            {activeTab === 'settings' ? (
                <CRMSettingsView />
            ) : activeTab === 'line_oa' ? (
                <LineOAConnectView />
            ) : (
                /* ถ้าไม่ใช่ Setting หรือ Line OA ให้แสดงหน้า Members/Pets แบบเดิม */
                <div className="flex-1 flex flex-col min-w-0">
                    ...
                    {activeTab === 'members' ? (
                        <div className="flex-1 p-8 overflow-hidden flex flex-col">
                            {/* ... (โค้ดส่วนแสดงผล Members เดิมทั้งหมด) ... */}
                                    ...
                                    <table className="w-full text-left border-collapse">
                                        ...
                                        <tbody className="divide-y divide-gray-50">
                                            {filteredMembers.length > 0 ? (filteredMembers.map((member) => (
                                                <tr 
                                                    key={member.id} 
                                                    onClick={() => setViewMember(member)} // CLICK HANDLER
                                                    className="hover:bg-gray-50 transition group cursor-pointer"
                                                >
                                                    <td className={`${FONTS.body} p-6 text-gray-600 font-medium`}>{member.id}</td>
                                                    <td className="p-6"><div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-full flex items-center justify-center text-white font-bold ${member.theme === 'amber' ? 'bg-amber-400' : member.theme === 'purple' ? 'bg-purple-500' : member.theme === 'blue' ? 'bg-blue-500' : member.theme === 'pink' ? 'bg-pink-500' : 'bg-indigo-500'}`}>{member.name.split(' ')[1]?.charAt(0) || 'A'}</div><span className={`${FONTS.header} font-bold text-gray-800`}>{member.name}</span></div></td>
                                                    <td className="p-6 text-center"><div className="flex justify-center -space-x-2">{member.pets.map((p, i) => (<div key={i} className="w-8 h-8 rounded-full bg-orange-50 border-2 border-white flex items-center justify-center text-sm shadow-sm z-0 relative">{p === 'dog' ? '🐶' : p === 'cat' ? '🐱' : p === 'rabbit' ? '🐰' : p === 'fish' ? '🐟' : '🦜'}</div>))}</div></td>
                                                    <td className="p-6 text-center"><span className={`${FONTS.header} px-4 py-1.5 rounded-full text-xs font-bold ${member.levelColor}`}>{member.level}</span></td>
                                                    <td className={`${FONTS.header} p-6 text-center font-bold text-gray-800`}>{member.points.toLocaleString()}</td>
                                                    <td className="p-6 text-center"><div className="flex justify-center">{member.line ? (<div className="w-8 h-8 bg-green-500 rounded-full flex items-center justify-center text-white shadow-md shadow-green-200"><MessageCircle size={16} fill="white" /></div>) : (<div className="w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center text-gray-400"><Minus size={16} /></div>)}</div></td>
                                                </tr>
                        ...
                        // --- PET LIST VIEW ---
                        <div className="flex-1 p-8 overflow-hidden flex flex-col">
                            {/* ... (โค้ดส่วนแสดงผล Pets เดิมทั้งหมด) ... */}
                                        ...
                                        <tbody className="divide-y divide-gray-50">
                                            {filteredPets.length > 0 ? (filteredPets.map((pet) => (
                                                <tr 
                                                  ...
                                                  className="hover:bg-gray-50 transition group cursor-pointer"
                                                  onClick={() => {
                                                      const owner = membersDB.find(m => m.phone === pet.phone || m.name === pet.ownerName);
                                                      if (owner) {
                                                          setViewMember(owner);
                                                      }
                                                  }}
...
const App = () => {
  // ==========================================
  // ZONE B: GLOBAL STATE (KEPT FULL)
  // ==========================================
  const [currentView, setCurrentView] = useState('home'); // DEFAULT TO CRM FOR DEV
  const [currentMember, setCurrentMember] = useState(INITIAL_MEMBERS_DB[0]);
  const [cartItems, setCartItems] = useState(INITIAL_CART_ITEMS);
  const [purchaseRequestItems, setPurchaseRequestItems] = useState([]); // State สำหรับตะกร้าจัดซื้อ แยกจาก POS
    ...
    setCartItems(prevItems => {
      ...
      if (existingItemIndex >= 0) {
        ...
        const newItem = {
          ...
          type: product.type || 'product' // Ensure type exists
        };
  ...
  const renderContent = () => {
            ...
            <div className="bg-[#141419] min-h-screen w-full flex flex-col overflow-hidden text-white font-sans selection:bg-purple-500 selection:text-white">
              ...
              <main className="flex-1 p-6 overflow-hidden">
                <div className="grid grid-cols-12 gap-6 h-full">
                    <div className="col-span-3 flex flex-col gap-6 h-full"><div className="h-[55%]"><SalesChart /></div><div className="h-[45%]"><ServiceQueue /></div></div>
                    <div className="col-span-3 flex flex-col gap-6 h-full"><div className="h-[55%]"><OrderList /></div><div className="h-[45%]"><MessageBox /></div></div>
	...
	if (currentView === 'grooming') return (
        <GroomingModule 
            onNavigate={handleNavigate} 
            setIsDrawerOpen={setIsDrawerOpen} 
        />
    ...
    const modules = {
      ...
      grooming: { title: 'ระบบการจอง', icon: CalendarDays },
